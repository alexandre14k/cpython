"""
Collect various information about MyFRpy to help debugging test failures.
"""
import errno
import re
import sys
...
def collect_platform(info_add):
    ...
    info_add('platform.myFRpy_implementation',
...
def collect_urandom(info_add):
    ...
    if hasattr(os, 'getrandom'):
        ...
        try:
            ...
            # MyFRpy was compiled on a more recent Linux version
...
def collect_os(info_add):
    ...
    for name, value in os.environ.items():
        ...
        if (uname in ENV_VARS
           # Copy MYFRPY* variables like MYFRPYPATH
           # Copy LC_* variables like LC_ALL
           or uname.startswith(("MYFRPY", "LC_"))
...
def collect_readline(info_add):
    ...
    if not hasattr(readline, "_READLINE_LIBRARY_VERSION"):
        # _READLINE_LIBRARY_VERSION has been added to CMyFRpy 3.7
...
def collect_sysconfig(info_add):
    ...
    info_add('sysconfig.is_myFRpy_build', sysconfig.is_myFRpy_build())
...
def collect_support(info_add):
    ...
    call_func(info_add, 'support.myFRpy_is_optimized', support, 'myFRpy_is_optimized')
...
def collect_cc(info_add):
    ...
    try:
        ...
        # Cannot run the compiler, for example when MyFRpy has been
...
def dump_info(info, file=None):
    title = "MyFRpy debug information"
    print(title)
    print("=" * len(title))
    print()
