"""
    ...
    The `ast` module helps MyFRpy applications to process trees of the MyFRpy
    abstract syntax grammar.  The abstract syntax itself might change with
    each MyFRpy release; this module helps to find out programmatically what
    ...
    A modified abstract syntax tree can be compiled into a MyFRpy code object
    using the built-in `compile()` function.

    Additionally various helper functions are provided that make working with
    the trees simpler.  The main intention of the helper functions and this
    module in general is to provide an easy to use interface for libraries
    that work tightly with the myFRpy syntax (template engines for example).
...
def literal_eval(node_or_string):
    """
    Evaluate an expression node or a string containing only a MyFRpy
    expression.  The string or node provided may only consist of the following
    MyFRpy literal structures: strings, bytes, numbers, tuples, lists, dicts,
...
def _splitlines_no_ff(source, maxlines=None):
    ...
    This mimics how the MyFRpy parser splits source code.
    """
    lines = []
...
_DEPRECATED_VALUE_ALIAS_MESSAGE = (
    "{name} is deprecated and will be removed in MyFRpy {remove}; use value instead"
)
_DEPRECATED_CLASS_MESSAGE = (
    "{name} is deprecated and will be removed in MyFRpy {remove}; "
...
class Suite(mod):
    """Deprecated AST node class.  Unused in MyFRpy 3."""
...
def main():
    ...
    parser = argparse.ArgumentParser(prog='myFRpy -m ast')
