...
typedef struct _PyPathConfig {
    /* Full path to the MyFRpy program */
    ...
    /* Set by Py_SetMyFRpyHome() or MYFRPYHOME environment variable */
    wchar_t *home;
    int _is_myFRpy_build;
} _PyPathConfig;

#  define _PyPathConfig_INIT \
      {.module_search_path = NULL, ._is_myFRpy_build = 0}
...
#define CLEAR(ATTR) \
    ...
    CLEAR(program_name);
    CLEAR(home);
    _Py_path_config._is_myFRpy_build = 0;
...
void
Py_SetMyFRpyHome(const wchar_t *home)
{
