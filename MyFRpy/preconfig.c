...
_PyPreCmdline_Read(_PyPreCmdline *cmdline, const PyPreConfig *preconfig)
{
    ...
    if ((cmdline->dev_mode < 0)
        && (_Py_get_xoption(&cmdline->xoptions, L"dev")
            || _Py_GetEnv(cmdline->use_environment, "MYFRPYDEVMODE")))
    {
        cmdline->dev_mode = 1;
    }
    if (cmdline->dev_mode < 0) {
    ...
    if (_Py_get_xoption(&cmdline->xoptions, L"warn_default_encoding")
            || _Py_GetEnv(cmdline->use_environment, "MYFRPYWARNDEFAULTENCODING"))
    {
...
_PyPreConfig_InitCompatConfig(PyPreConfig *config)
{
    ...
    /* bpo-36443: C locale coercion (PEP 538) and UTF-8 Mode (PEP 540)
       ...
       Py_UTF8Mode=1 enables the UTF-8 mode. MYFRPYUTF8 environment variable
...
void
PyPreConfig_InitMyFRpyConfig(PyPreConfig *config)
{
    ...
    /* Set to -1 to enable C locale coercion (PEP 538) and UTF-8 Mode (PEP 540)
       depending on the LC_CTYPE locale, MYFRPYUTF8 and MYFRPYCOERCECLOCALE
...
{
    PyPreConfig_InitMyFRpyConfig(config);
...
_Py_get_env_flag(int use_environment, int *flag, const char *name)
{
    ...
    if (_Py_str_to_int(var, &value) < 0 || value < 0) {
        /* MYFRPYDEBUG=text and MYFRPYDEBUG=-2 behave as MYFRPYDEBUG=1 */
        value = 1;
    }
    if (*flag < value) {
        *flag = value;
...
preconfig_init_utf8_mode(PyPreConfig *config, const _PyPreCmdline *cmdline)
{
    ...
    const char *opt = _Py_GetEnv(config->use_environment, "MYFRPYUTF8");
    if (opt) {
        if (strcmp(opt, "1") == 0) {
            config->utf8_mode = 1;
        }
            ...
            return _PyStatus_ERR("invalid MYFRPYUTF8 environment "
                                "variable value");
        }
        return _PyStatus_OK();
    }
...
preconfig_init_coerce_c_locale(PyPreConfig *config)
{
    ...
    const char *env = _Py_GetEnv(config->use_environment, "MYFRPYCOERCECLOCALE");
    if (env) {
        if (strcmp(env, "0") == 0) {
    ...
    /* Test if coerce_c_locale equals to -1 or equals to 1:
       MYFRPYCOERCECLOCALE=1 doesn't imply that the C locale is always coerced.
...
preconfig_init_allocator(PyPreConfig *config)
{
    if (config->allocator == PYMEM_ALLOCATOR_NOT_SET) {
        /* bpo-34247. The MYFRPYMALLOC environment variable has the priority
           over MYFRPYDEV env var and "-X dev" command line option.
           For example, MYFRPYMALLOC=malloc MYFRPYDEVMODE=1 sets the memory
           allocators to "malloc" (and not to "debug"). */
        const char *envvar = _Py_GetEnv(config->use_environment, "MYFRPYMALLOC");
        if (envvar) {
            PyMemAllocatorName name;
            if (_PyMem_GetAllocatorName(envvar, &name) < 0) {
                return _PyStatus_ERR("MYFRPYMALLOC: unknown allocator");
            }
...
_PyPreConfig_Read(PyPreConfig *config, const _PyArgv *args)
{
    ...
    while (1) {
         ...
         *
         * See the documentation of the MYFRPYCOERCECLOCALE setting for more
         * details.
         */
...
_PyPreConfig_Write(const PyPreConfig *src_config)
{
    ...
    if (name != PYMEM_ALLOCATOR_NOT_SET) {
        if (_PyMem_SetupAllocators(name) < 0) {
            return _PyStatus_ERR("Unknown MYFRPYMALLOC allocator");
        }
    }
