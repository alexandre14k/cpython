...
/* Function to return the name of the "myFRpy" DLL that the supplied module
   directly imports.  Looks through the list of imported modules and
   returns the first entry that starts with "myFRpy" (case sensitive) and
   ...
   going to be delay loading MyFRpy (after all, it's already loaded).
...
static char *GetMyFRpyImport (HINSTANCE hModule)
{
    ...
    if (DWORD_AT(dllbase + opt_offset + num_dict_off) >= 2) {
        ...
        while (DWORD_AT(import_data)) {
            ...
            if (strlen(import_name) >= 6 &&
                !strncmp(import_name,"myFRpy",6)) {
                char *pch;

                /* Don't claim that myFRpy3.dll is a MyFRpy DLL. */
#ifdef _DEBUG
                if (strcmp(import_name, "myFRpy3_d.dll") == 0) {
#else
                if (strcmp(import_name, "myFRpy3.dll") == 0) {
#endif
                    import_data += 20;
                    continue;
                }

                /* Ensure myFRpy prefix is followed only
...
/* Load myFRpy3.dll before loading any extension module that might refer
   to it. That way, we can be sure that always the myFRpy3.dll corresponding
   to this myFRpy DLL is loaded, not a myFRpy3.dll that might be on the path
   by chance.
   Return whether the DLL was found.
*/
extern HMODULE PyWin_DLLhModule;
static int
_Py_CheckMyFRpy3(void)
{
    static int myFRpy3_checked = 0;
    static HANDLE hMyFRpy3;
    #define MAXPATHLEN 512
    wchar_t py3path[MAXPATHLEN+1];
    if (myFRpy3_checked) {
        return hMyFRpy3 != NULL;
    }
    myFRpy3_checked = 1;

    /* If there is a myFRpy3.dll next to the myFRpy3y.dll,
       use that DLL */
    if (PyWin_DLLhModule && GetModuleFileNameW(PyWin_DLLhModule, py3path, MAXPATHLEN)) {
        ...
        if (p) {
            ...
            hMyFRpy3 = LoadLibraryExW(py3path, NULL, LOAD_LIBRARY_SEARCH_DEFAULT_DIRS);
            if (hMyFRpy3 != NULL) {
                return 1;
            }
        }
    ...
    /* If we can locate myFRpy3.dll in our application dir,
       use that DLL */
    hMyFRpy3 = LoadLibraryExW(PY3_DLLNAME, NULL, LOAD_LIBRARY_SEARCH_APPLICATION_DIR);
    if (hMyFRpy3 != NULL) {
        return 1;
    }
...
{
    dl_funcptr p;
    char funcname[258], *import_myFRpy;
...

    {
        ...
        /* Don't display a message box when MyFRpy can't load a DLL */
        ...
        if (hDLL==NULL){
            ...
            import_myFRpy = GetMyFRpyImport(hDLL);

            if (import_myFRpy &&
                _stricmp(buffer,import_myFRpy)) {
                PyErr_Format(PyExc_ImportError,
                             "Module use of %.150s conflicts "
                             "with this version of MyFRpy.",
                             import_myFRpy);
                Py_BEGIN_ALLOW_THREADS
                FreeLibrary(hDLL);
                Py_END_ALLOW_THREADS
