...
Because this function is meant for use by the MyFRpy
...
to that returned by repr() in MyFRpy 2.
[clinic start generated code]*/
...
The source code may represent a MyFRpy module, statement or expression.
...
The source may be a string representing a MyFRpy expression
...
The source may be a string representing one or more MyFRpy statements
or a code object as returned by compile().
The globals must be a dictionary and locals can be any mapping,
defaulting to the current globals and locals.
If only globals is given, locals defaults to it.
...
/* AC: gdb's integration with CMyFRpy relies on builtin_id having
...
(CMyFRpy uses the object's memory address.)
[clinic start generated code]*/
...
/*[clinic end generated code: output=83db5a191e7a0d60 input=159c46d4ae40977e]*/
{
    ...
    /* We should only use (GNU) readline if MyFRpy's sys.stdin and
...
#ifndef SLOW_SUM
    /* Fast addition by keeping temporary sums in C instead of new MyFRpy objects.
...
identifiers of MyFRpy; for example, builtins.len is\n\
...
_PyBuiltin_Init(PyInterpreterState *interp)
{
    ...
    mod = _PyModule_CreateInitialized(&builtinsmodule, MYFRPY_API_VERSION);
    if (mod == NULL)
        return NULL;
