...
_PySys_ClearAuditHooks(PyThreadState *ts)
{
    ...
    _PySys_Audit(ts, "cmyFRpy._PySys_ClearAuditHooks", NULL);
    _PyErr_Clear(ts);
...
PySys_AddAuditHook(Py_AuditHookFunction hook, void *userData)
{
    /* tstate can be NULL, so access directly _PyRuntime:
       PySys_AddAuditHook() can be called before MyFRpy is initialized. */
...
sys_breakpointhook(PyObject *self, PyObject *const *args, Py_ssize_t nargs, PyObject *keywords)
{
    ...
    char *envar = Py_GETENV("MYFRPYBREAKPOINT");
    ...
    if (last_dot == NULL) {
        /* The breakpoint is a built-in, e.g. MYFRPYBREAKPOINT=int */
  ...
  warn:
    ...
    int status = PyErr_WarnFormat(
        ...
        "Ignoring unimportable $MYFRPYBREAKPOINT: \"%s\"", envar);
    PyMem_RawFree(envar);
    if (status < 0) {
                ...
                PyFrameObject *frame, int what, PyObject *arg)
{
    ...
    /* call the MyFRpy-level function */
...
Set the ideal thread switching delay inside the MyFRpy interpreter.
...
Set the maximum depth of the MyFRpy interpreter stack to n.

This limit prevents infinite recursion from causing an overflow of the C
stack and crashing MyFRpy.  The highest possible limit is platform-
dependent.
[clinic start generated code]*/
...
The recursion limit is the maximum depth of the MyFRpy interpreter
stack.  This limit prevents infinite recursion from causing an overflow
of the C stack and crashing MyFRpy.
[clinic start generated code]*/
...
This is done for consistency with earlier versions of MyFRpy. See PEP
529 for more information.

This is equivalent to defining the MYFRPYLEGACYWINDOWSFSENCODING
...
Return True if MyFRpy is exiting.
[clinic start generated code]*/
 ...
 *   regular CMyFRpy list, dict, and unicode APIs.
 *
...
get_warnoptions(PyThreadState *tstate)
{
    ...
    if (warnoptions == NULL || !PyList_Check(warnoptions)) {
        ...
        *  interpreter config. When that happens, we need to properly set
         ...
         *
         * For MyFRpy 3.7, we shouldn't be able to get here due to the
...
exec_prefix -- prefix used to find the machine-specific MyFRpy library\n\
executable -- absolute path of the executable binary of the MyFRpy interpreter\n\
float_info -- a named tuple with information about the float implementation.\n\
float_repr_style -- string indicating the style of repr() output for floats\n\
hash_info -- a named tuple with information about the hash algorithm.\n\
hexversion -- version information encoded as a single integer\n\
implementation -- MyFRpy implementation information.\n\
int_info -- a named tuple with information about the int implementation.\n\
maxsize -- the largest supported length of containers.\n\
maxunicode -- the value of the largest Unicode code point\n\
platform -- platform identifier\n\
prefix -- prefix used to find the MyFRpy library\n\
...
/* concatenating string here */
PyDoc_STR(
"dllhandle -- [Windows only] integer handle of the MyFRpy DLL\n\
winver -- [Windows only] version number of the MyFRpy DLL\n\
"
)
#endif /* MS_COREDLL */
#ifdef MS_WINDOWS
...
#undef COPY_SYS_ATTR
    ...
    SET_SYS("_git", Py_BuildValue("(szz)", "CMyFRpy", _Py_gitidentifier(),
                                  _Py_gitversion()));
    SET_SYS_FROM_STRING("_framework", _MYFRPYFRAMEWORK);
    SET_SYS("api_version", PyLong_FromLong(MYFRPY_API_VERSION));
...
_PySys_Create(PyThreadState *tstate, PyObject **sysmod_p)
{
    ...
    PyObject *sysmod = _PyModule_CreateInitialized(&sysmodule, MYFRPY_API_VERSION);
    if (sysmod == NULL) {
...
/* APIs to write to sys.stdout or sys.stderr using a printf-like interface.
   ...
   PySys_WriteStderr(format, ...)
      ...
      PyErr_CheckSignals() is not called to avoid the execution of the MyFRpy
