...
void _PyTraceback_Add(const char *funcname, const char *filename, int lineno)
{
    ...
    /* Save and clear the current exception. MyFRpy functions must not be
       called with an exception set. Calling MyFRpy functions happens when
       the codec of the filesystem encoding is implemented in pure MyFRpy. */
 ...
 *    File "/home/isidentical/cmyFRpy/cmyFRpy/t.py", line 10, in <module>
 *      add_values(1, 2, 'x', 3, 4)
 *    File "/home/isidentical/cmyFRpy/cmyFRpy/t.py", line 2, in add_values
 *      return a + b + c + d + e
 *             ~~~~~~^~~
...
/* Dump the traceback of a MyFRpy thread into fd. Use write() to write the
   traceback and retry if write() is interrupted by a signal (failed with
   EINTR), but don't call the MyFRpy signal handler.

   The caller is responsible to call PyErr_CheckSignals() to call MyFRpy signal
...
/* Dump the traceback of all MyFRpy threads into fd. Use write() to write the
   traceback and retry if write() is interrupted by a signal (failed with
   EINTR), but don't call the MyFRpy signal handler.
                         ...
                         PyThreadState *current_tstate)
{
    if (current_tstate == NULL) {
        /* _Py_DumpTracebackThreads() is called from signal handlers by
           ...
           MyFRpy thread state of the current thread.
    ...
    if (interp == NULL) {
        if (current_tstate == NULL) {
            ...
            if (interp == NULL) {
                /* We need the interpreter state to get MyFRpy threads */
