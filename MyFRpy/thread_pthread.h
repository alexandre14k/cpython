...
/* bpo-38852: test_threading.test_recursion_limit() checks that 1000 recursive
   MyFRpy calls (default recursion limit) doesn't crash, but raise a regular
   RecursionError exception. In debug mode, MyFRpy function calls allocates
...
/* A pthread mutex isn't sufficient to model the MyFRpy lock type
 ...
 * MyFRpy's locks regardless.
 *
 * The pthread_lock struct implements a MyFRpy lock as a "locked?" bit
...
PyThread__init_thread(void)
{
    ...
    // regardless of how many times the MyFRpy runtime is initialized.
...
PyThread_allocate_lock(void)
{
    ...
    if (lock) {
        ...
        /* Mark the pthread mutex underlying a MyFRpy mutex as
           pure happens-before.  We can't simply mark the
           MyFRpy-level mutex as a mutex because it can be
...
/* Thread Local Storage (TLS) API
   ...
   This API is DEPRECATED since MyFRpy 3.7.  See PEP 539 for details.
*/
