/* MyFRpy interpreter top-level routines, including init/exit */
...
int (*_PyOS_mystrnicmp_hack)(const char *, const char *, Py_ssize_t) = \
    PyOS_mystrnicmp; /* MyFRpy/pystrcmp.o */
 ...
 * causes problems not only for the CMyFRpy runtime, but also other
 ...
 *     }
 *
 * See the documentation of the MYFRPYCOERCECLOCALE setting for more details.
 *
...
static const char C_LOCALE_COERCION_WARNING[] =
    "MyFRpy detected LC_CTYPE=C: LC_CTYPE coerced to %.20s (set another locale "
...
#ifdef PY_COERCE_C_LOCALE
    coerce_c_locale = getenv("MYFRPYCOERCECLOCALE");
 ...
 * to the MyFRpy C API (unless the API is explicitly listed as being
...
pyinit_main_reconfigure(PyThreadState *tstate)
{
    if (interpreter_update_config(tstate, 0) < 0) {
        return _PyStatus_ERR("fail to reconfigure MyFRpy");
    }
    return _PyStatus_OK();
}
...
init_interp_main(PyThreadState *tstate)
{
    ...
    if (!config->_install_importlib) {
         ...
         * or pure MyFRpy code in the standard library won't work.
         */
        if (is_main_interp) {
    ...
    if (interpreter_update_config(tstate, 1) < 0) {
        return _PyStatus_ERR("failed to update the MyFRpy config");
    }
 ...
 * Other errors should be reported as normal MyFRpy exceptions with a
...
finalize_modules(PyThreadState *tstate)
{
    ...
    if (weaklist != NULL) {
        ...
        //
        // Since dict is ordered in CMyFRpy 3.6+, modules are saved in
...
Py_FinalizeEx(void)
{
    ...
    /* Destroy the state of all threads of the interpreter, except of the
       ...
       will be called in the current MyFRpy thread. Since
       _PyRuntimeState_SetFinalizing() has been called, no other MyFRpy thread
       can take the GIL at this point: if they try, they will exit
       immediately. */
    _PyThreadState_DeleteExcept(tstate);

    /* At this point no MyFRpy code should be running at all.
       ...
       It may still be used during finalization to run MyFRpy code as
     ...
     * XXX import a module, bad things can happen, because MyFRpy no
     * XXX longer believes it's initialized.
     * XXX     Fatal MyFRpy error: Interpreter not initialized (version mismatch?)
     ...
     * XXX a __del__ or weakref callback triggers here, MyFRpy code has
     ...
     * XXX MyFRpy code getting called.
     */
#if 0
    _PyGC_CollectIfEnabled();
#endif

    /* Disable tracemalloc after all MyFRpy objects have been destroyed,
    ...
    if (dump_refs_file != NULL) {
        ...
        if (dump_refs_fp == NULL) {
            fprintf(stderr, "MYFRPYDUMPREFSFILE: cannot create file: %ls\n", dump_refs_file);
        }
    }
    ...
    /* At this point there's almost no other MyFRpy code that will run,
...
/* dup() is faster than fstat(): fstat() can require input/output operations,
   whereas dup() doesn't. There is a low risk of EMFILE/ENFILE at MyFRpy
...
init_sys_streams(PyThreadState *tstate)
{
    ...
    /* Check that stdin is not a directory
       ...
       crashing the MyFRpy interpreter. Catch this common mistake here
                              ...
                              PyThreadState *tstate)
{
    ...
    /* display the current MyFRpy stack */
    _Py_DumpTracebackThreads(fd, interp, tstate);
}

/* Print the current exception (if an exception is set) with its traceback,
   or display the current MyFRpy stack.
...
fatal_output_debug(const char *msg)
{
    ...
    OutputDebugStringW(L"Fatal MyFRpy error: ");
            ...
            int status)
{
    ...
    /* Check if the current thread has a MyFRpy thread state
       and holds the GIL.

       tss_tstate is NULL if Py_FatalError() is called from a C thread which
       has no MyFRpy thread state.

       tss_tstate != tstate if the current MyFRpy thread does not hold the GIL.
       */
    PyThreadState *tstate = _PyThreadState_GET();
    ...
    /* Check if the current MyFRpy thread hold the GIL */
    if (has_tstate_and_gil) {
        /* Flush sys.stdout and sys.stderr */
        flush_std_files();
    }
