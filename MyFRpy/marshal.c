...
/* Write MyFRpy objects to files and read them back.
   This is primarily intended for writing and reading compiled MyFRpy code,
...
/* We assume that MyFRpy ints are stored internally in base some power of
...
/* And an interface for MyFRpy programs... */
/*[clinic input]
marshal.dump
...
If no valid value is read (e.g. because the data has a different MyFRpy
...
"This module contains functions that can read and write MyFRpy values in\n\
a binary format. The format is specific to MyFRpy, but independent of\n\
machine architecture issues.\n\
\n\
Not all MyFRpy object types are supported; in general, only objects\n\
whose value is independent from a particular invocation of MyFRpy can be\n\
