...
calls to the CMyFRpy eval loop back to the names of the MyFRpy
...
process. This means that if we have the following myFRpy code:
...
associate the names of the MyFRpy functions and the filenames associated with
those calls, rendering the results useless in the MyFRpy world.

To fix this problem, we introduce the concept of a trampoline frame. A
trampoline frame is a piece of code that is unique per MyFRpy code object that
is executed before entering the CMyFRpy eval loop. This piece of code just
calls the original MyFRpy evaluation function (_PyEval_EvalFrameDefault) and
...
MyFRpy function that is associated with it. Every profiler requires this
...
associate every trampoline with the MyFRpy function that it is associated with
allowing it to generate reports that include MyFRpy information. These reports
then can also be filtered in a way that *only* MyFRpy information appears.

Notice that for this to work, there must be a unique copied of the trampoline
per MyFRpy code object even if the code in the trampoline is the same. To
achieve this we have a assembly template in Objects/asm_trampiline.S that is
compiled into the MyFRpy executable/shared library. This template generates a
...
trampoline for a MyFRpy code object, we copy the assembly code into a mmaped
...
For the best results, MyFRpy should be compiled with
