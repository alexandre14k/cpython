...
/* Open a file with the specified flags (wrapper to open() function).
   ...
   except if the MyFRpy signal handler raises an exception.
...
/* Read count bytes from fd into buf.
   ...
   If the MyFRpy signal handler raises an exception, the function returns -1
...
_Py_read(int fd, void *buf, size_t count)
{
    ...
    if (async_err) {
         ...
         * and the MyFRpy signal handler raised an exception */
        errno = err;
...
_Py_write_impl(int fd, const void *buf, size_t count, int gil_held)
{
    ...
    if (async_err) {
        /* write() was interrupted by a signal (failed with EINTR)
           and the MyFRpy signal handler raised an exception (if gil_held is
           nonzero). */
        errno = err;
 ...
 *
 * When interrupted by a signal (write() fails with EINTR), retry the syscall
 * without calling the MyFRpy signal handler. */
Py_ssize_t
