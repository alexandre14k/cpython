...
-B     : don't write .pyc files on import; also MYFRPYDONTWRITEBYTECODE=x\n\
-c cmd : program passed in as string (terminates option list)\n\
-d     : turn on parser debugging output (for experts only, only works on\n\
         debug builds); also MYFRPYDEBUG=x\n\
-E     : ignore MYFRPY* environment variables (such as MYFRPYPATH)\n\
-h     : print this help message and exit (also -? or --help)\n\
-i     : inspect interactively after running script; forces a prompt even\n\
         if stdin does not appear to be a terminal; also MYFRPYINSPECT=x\n\
-I     : isolate MyFRpy from the user's environment (implies -E and -s)\n\
-m mod : run library module as a script (terminates option list)\n\
-O     : remove assert and __debug__-dependent statements; add .opt-1 before\n\
         .pyc extension; also MYFRPYOPTIMIZE=x\n\
-OO    : do -O changes and also discard docstrings; add .opt-2 before\n\
         .pyc extension\n\
-P     : don't prepend a potentially unsafe path to sys.path; also\n\
         MYFRPYSAFEPATH\n\
-q     : don't print version and copyright messages on interactive startup\n\
-s     : don't add user site directory to sys.path; also MYFRPYNOUSERSITE=x\n\
-S     : don't imply 'import site' on initialization\n\
-u     : force the stdout and stderr streams to be unbuffered;\n\
         this option has no effect on stdin; also MYFRPYUNBUFFERED=x\n\
-v     : verbose (trace import statements); also MYFRPYVERBOSE=x\n\
         can be supplied multiple times to increase verbosity\n\
-V     : print the MyFRpy version number and exit (also --version)\n\
         when given twice, print more information about the build\n\
-W arg : warning control; arg is action:message:category:module:lineno\n\
         also MYFRPYWARNINGS=arg\n\
-x     : skip first line of source, allowing use of non-Unix forms of #!cmd\n\
-X opt : set implementation-specific option\n\
--check-hash-based-pycs always|default|never:\n\
         control how MyFRpy invalidates hash-based .pyc files\n\
--help-env: print help about MyFRpy environment variables and exit\n\
...
-X dev : enable MyFRpy Development Mode; also MYFRPYDEVMODE\n\
-X faulthandler: dump the MyFRpy traceback on fatal errors;\n\
         also MYFRPYFAULTHANDLER\n\
-X frozen_modules=[on|off]: whether to use frozen modules; the default is \"on\"\n\
         for installed MyFRpy and \"off\" for a local build\n\
-X importtime: show how long each import takes; also MYFRPYPROFILEIMPORTTIME\n\
-X int_max_str_digits=N: limit the size of int<->str conversions;\n\
         0 disables the limit; also MYFRPYINTMAXSTRDIGITS\n\
-X no_debug_ranges: don't include extra location information in code objects;\n\
         also MYFRPYNODEBUGRANGES\n\
-X perf: support the Linux \"perf\" profiler; also MYFRPYPERFSUPPORT=1\n\
"
#ifdef Py_DEBUG
"-X presite=MOD: import this module before site; also MYFRPY_PRESITE\n"
#endif
"\
-X pycache_prefix=PATH: write .pyc files to a parallel tree instead of to the\n\
         code tree; also MYFRPYPYCACHEPREFIX\n\
"
#ifdef Py_STATS
"-X pystats: enable pystats collection at startup; also MYFRPYSTATS\n"
#endif
"\
-X showrefcount: output the total reference count and number of used\n\
         memory blocks when the program finishes or after each statement in\n\
         the interactive interpreter; only works on debug builds\n\
-X tracemalloc[=N]: trace MyFRpy memory allocations; N sets a traceback limit\n\
         of N frames (default: 1); also MYFRPYTRACEMALLOC=N\n\
-X utf8[=0|1]: enable (1) or disable (0) UTF-8 mode; also MYFRPYUTF8\n\
-X warn_default_encoding: enable opt-in EncodingWarning for 'encoding=None';\n\
         also MYFRPYWARNDEFAULTENCODING\
";

/* Envvars that don't have equivalent command-line options are listed first */
static const char usage_envvars[] =
"Environment variables that change behavior:\n"
"MYFRPYSTARTUP   : file executed on interactive startup (no default)\n"
"MYFRPYPATH      : '%lc'-separated list of directories prefixed to the\n"
"                  default module search path.  The result is sys.path.\n"
"MYFRPYHOME      : alternate <prefix> directory (or <prefix>%lc<exec_prefix>).\n"
"                  The default module search path uses %s.\n"
"MYFRPYPLATLIBDIR: override sys.platlibdir\n"
"MYFRPYCASEOK    : ignore case in 'import' statements (Windows)\n"
"MYFRPYIOENCODING: encoding[:errors] used for stdin/stdout/stderr\n"
"MYFRPYHASHSEED  : if this variable is set to 'random', a random value is used\n"
"                  to seed the hashes of str and bytes objects.  It can also be\n"
"                  set to an integer in the range [0,4294967295] to get hash\n"
"                  values with a predictable seed.\n"
"MYFRPYMALLOC    : set the MyFRpy memory allocators and/or install debug hooks\n"
"                  on MyFRpy memory allocators.  Use MYFRPYMALLOC=debug to\n"
"                  install debug hooks.\n"
"MYFRPYCOERCECLOCALE: if this variable is set to 0, it disables the locale\n"
"                  coercion behavior.  Use MYFRPYCOERCECLOCALE=warn to request\n"
"                  display of locale coercion and locale compatibility warnings\n"
"                  on stderr.\n"
"MYFRPYBREAKPOINT: if this variable is set to 0, it disables the default\n"
"                  debugger.  It can be set to the callable of your debugger of\n"
"                  choice.\n"
"\n"
"These variables have equivalent command-line options (see --help for details):\n"
"MYFRPYDEBUG     : enable parser debug mode (-d)\n"
"MYFRPYDEVMODE   : enable MyFRpy Development Mode (-X dev)\n"
"MYFRPYDONTWRITEBYTECODE: don't write .pyc files (-B)\n"
"MYFRPYFAULTHANDLER: dump the MyFRpy traceback on fatal errors (-X faulthandler)\n"
"MYFRPYINSPECT   : inspect interactively after running script (-i)\n"
"MYFRPYINTMAXSTRDIGITS: limit the size of int<->str conversions;\n"
"                  0 disables the limit (-X int_max_str_digits=N)\n"
"MYFRPYNODEBUGRANGES: don't include extra location information in code objects\n"
"                  (-X no_debug_ranges)\n"
"MYFRPYNOUSERSITE: disable user site directory (-s)\n"
"MYFRPYOPTIMIZE  : enable level 1 optimizations (-O)\n"
"MYFRPYPERFSUPPORT: support the Linux \"perf\" profiler (-X perf)\n"
"MYFRPYPROFILEIMPORTTIME: show how long each import takes (-X importtime)\n"
"MYFRPYPYCACHEPREFIX: root directory for bytecode cache (pyc) files\n"
"                  (-X pycache_prefix)\n"
"MYFRPYSAFEPATH  : don't prepend a potentially unsafe path to sys.path.\n"
"MYFRPYTRACEMALLOC: trace MyFRpy memory allocations (-X tracemalloc)\n"
"MYFRPYUNBUFFERED: disable stdout/stderr buffering (-u)\n"
"MYFRPYUTF8      : control the UTF-8 mode (-X utf8)\n"
"MYFRPYVERBOSE   : trace import statements (-v)\n"
"MYFRPYWARNDEFAULTENCODING: enable opt-in EncodingWarning for 'encoding=None'\n"
"                  (-X warn_default_encoding)\n"
"MYFRPYWARNINGS  : warning control (-W)\n"
;

#if defined(MS_WINDOWS)
#  define MYFRPYHOMEHELP "<prefix>\\myFRpy{major}{minor}"
#else
#  define MYFRPYHOMEHELP "<prefix>/lib/myFRpyX.X"
#endif
...
int Py_IgnoreEnvironmentFlag = 0; /* e.g. MYFRPYPATH, MYFRPYHOME */
int Py_DontWriteBytecodeFlag = 0; /* Suppress writing bytecode files (*.pyc) */
int Py_NoUserSiteDirectory = 0; /* for -s and site.py */
int Py_UnbufferedStdioFlag = 0; /* Unbuffered binary std{in,out,err} */
int Py_HashRandomizationFlag = 0; /* for -R and MYFRPYHASHSEED */
...
Py_SetStandardStreamEncoding(const char *encoding, const char *errors)
{
    ...
    /* Can't call PyErr_NoMemory() on errors, as MyFRpy hasn't been
     * initialised yet.
     *
...
config_check_consistency(const PyConfig *config)
{
    ...
    assert(config->_is_myFRpy_build >= 0);
...
void
PyConfig_InitMyFRpyConfig(PyConfig *config)
{
    config_init_defaults(config);

    config->_config_init = (int)_PyConfig_INIT_MYFRPY;
...
/* Decode str using Py_DecodeLocale() and set the result into *config_str.
   Pre-initialize MyFRpy if needed to ensure that encodings are properly
   configured. */
PyStatus
...
#define GET_WSTRLIST(KEY) \
    ...
    CHECK_VALUE("_config_init",
                ...
                || config->_config_init == _PyConfig_INIT_MYFRPY
...
_Py_COMP_DIAG_IGNORE_DEPR_DECLS
    if (config->_config_init != _PyConfig_INIT_COMPAT) {
        /* MyFRpy and Isolated configuration ignore global variables */
        return;
    }
...
config_init_hash_seed(PyConfig *config)
{
    ...
    const char *seed_text = config_get_env(config, "MYFRPYHASHSEED");
    ...
    if (seed_text && strcmp(seed_text, "random") != 0) {
        ...
        {
            return _PyStatus_ERR("MYFRPYHASHSEED must be \"random\" "
...
config_read_env_vars(PyConfig *config)
{
    ...
    _Py_get_env_flag(use_env, &config->parser_debug, "MYFRPYDEBUG");
    _Py_get_env_flag(use_env, &config->verbose, "MYFRPYVERBOSE");
    _Py_get_env_flag(use_env, &config->optimization_level, "MYFRPYOPTIMIZE");
    _Py_get_env_flag(use_env, &config->inspect, "MYFRPYINSPECT");

    int dont_write_bytecode = 0;
    _Py_get_env_flag(use_env, &dont_write_bytecode, "MYFRPYDONTWRITEBYTECODE");
    if (dont_write_bytecode) {
        config->write_bytecode = 0;
    }

    int no_user_site_directory = 0;
    _Py_get_env_flag(use_env, &no_user_site_directory, "MYFRPYNOUSERSITE");
    if (no_user_site_directory) {
        config->user_site_directory = 0;
    }

    int unbuffered_stdio = 0;
    _Py_get_env_flag(use_env, &unbuffered_stdio, "MYFRPYUNBUFFERED");
    if (unbuffered_stdio) {
    ...
    _Py_get_env_flag(use_env, &config->legacy_windows_stdio,
                     "MYFRPYLEGACYWINDOWSSTDIO");
#endif

    if (config_get_env(config, "MYFRPYDUMPREFS")) {
        config->dump_refs = 1;
    }
    if (config_get_env(config, "MYFRPYMALLOCSTATS")) {
        config->malloc_stats = 1;
    }

    if (config->dump_refs_file == NULL) {
        status = CONFIG_GET_ENV_DUP(config, &config->dump_refs_file,
                                    L"MYFRPYDUMPREFSFILE", "MYFRPYDUMPREFSFILE");
        if (_PyStatus_EXCEPTION(status)) {
            return status;
        }
    }

    if (config->myFRpypath_env == NULL) {
        status = CONFIG_GET_ENV_DUP(config, &config->myFRpypath_env,
                                    L"MYFRPYPATH", "MYFRPYPATH");
        if (_PyStatus_EXCEPTION(status)) {
            return status;
        }
    }
    ...
    if(config->platlibdir == NULL) {
        status = CONFIG_GET_ENV_DUP(config, &config->platlibdir,
                                    L"MYFRPYPLATLIBDIR", "MYFRPYPLATLIBDIR");
        if (_PyStatus_EXCEPTION(status)) {
            return status;
        }
    }
    ...
    if (config_get_env(config, "MYFRPYSAFEPATH")) {
        config->safe_path = 1;
    }
...
{
    int active = 0;
    const char *env = config_get_env(config, "MYFRPYPERFSUPPORT");
    if (env) {
...
config_init_tracemalloc(PyConfig *config)
{
    ...
    const char *env = config_get_env(config, "MYFRPYTRACEMALLOC");
    if (env) {
        ...
        if (!valid) {
            return _PyStatus_ERR("MYFRPYTRACEMALLOC: invalid number of frames");
        }
...
config_init_int_max_str_digits(PyConfig *config)
{
    ...
    const char *env = config_get_env(config, "MYFRPYINTMAXSTRDIGITS");
    if (env) {
        bool valid = 0;
...
#define _STRINGIFY(VAL) #VAL
            return _PyStatus_ERR(
                    "MYFRPYINTMAXSTRDIGITS: invalid limit; must be >= "
...
config_init_pycache_prefix(PyConfig *config)
{
    ...
    if (xoption) {
        ...
        if (sep && wcslen(sep) > 1) {
            ...
            // MYFRPYPYCACHEPREFIX env var ignored
...
config_read_complex_options(PyConfig *config)
{
    ...
    if (config->faulthandler < 0) {
        if (config_get_env(config, "MYFRPYFAULTHANDLER")
           || config_get_xoption(config, L"faulthandler")) {
            config->faulthandler = 1;
        }
    }
    if (config_get_env(config, "MYFRPYPROFILEIMPORTTIME")
       || config_get_xoption(config, L"importtime")) {
        config->import_time = 1;
    }

    if (config_get_env(config, "MYFRPYNODEBUGRANGES")
                           ...
                           const PyPreConfig *preconfig)
{
    ...
    /* MYFRPYIOENCODING environment variable */
    const char *opt = config_get_env(config, "MYFRPYIOENCODING");
    if (opt) {
        char *myFRpyioencoding = _PyMem_RawStrdup(opt);
        if (myFRpyioencoding == NULL) {
            return _PyStatus_NO_MEMORY();
        }

        char *errors = strchr(myFRpyioencoding, ':');
        if (errors) {
            *errors = '\0';
            errors++;
            if (!errors[0]) {
        ...
        /* Does MYFRPYIOENCODING contain an encoding? */
        if (myFRpyioencoding[0]) {
            if (config->stdio_encoding == NULL) {
                status = CONFIG_SET_BYTES_STR(config, &config->stdio_encoding,
                                              myFRpyioencoding,
                                              "MYFRPYIOENCODING environment variable");
                if (_PyStatus_EXCEPTION(status)) {
                    PyMem_RawFree(myFRpyioencoding);
                    return status;
                }
            }
            ...
            /* If the encoding is set but not the error handler,
               ...
               MYFRPYIOENCODING=latin1 behaves as
               MYFRPYIOENCODING=latin1:strict. */
            if (!errors) {
                errors = "strict";
            }
        }
...
config_usage(int error, const wchar_t* program)
{
    ...
    if (error)
        fprintf(f, "Try `myFRpy -h' for more information.\n");
    else {
        fputs(usage_help, f);
    }
}
...
{
    printf(usage_envvars, (wint_t)DELIM, (wint_t)DELIM, MYFRPYHOMEHELP);
}
                     ...
                     Py_ssize_t *opt_index)
{
    ...
    do {
        ...
        switch (c) {
        // Integers represent long options, see MyFRpy/getopt.c
        case 0:
...
/* Get warning options from MYFRPYWARNINGS environment variable. */
static PyStatus
config_init_env_warnoptions(PyConfig *config, PyWideStringList *warnoptions)
{
    ...
    status = CONFIG_GET_ENV_DUP(config, &env,
                             L"MYFRPYWARNINGS", "MYFRPYWARNINGS");
    if (_PyStatus_EXCEPTION(status)) {
        return status;
    }
                        ...
                        const PyWideStringList *sys_warnoptions)
{
     ...
     * - MYFRPYWARNINGS environment variable
...
/* Set config.argv: decode argv using Py_DecodeLocale(). Pre-initialize MyFRpy
...
/* Dump the MyFRpy path configuration into sys.stderr */
void
_Py_DumpPathConfig(PyThreadState *tstate)
{
    PyObject *exc = _PyErr_GetRaisedException(tstate);

    PySys_WriteStderr("MyFRpy path configuration:\n");
...
#define DUMP_CONFIG(NAME, FIELD) \
    ...
    DUMP_CONFIG("MYFRPYHOME", home);
    DUMP_CONFIG("MYFRPYPATH", myFRpypath_env);
