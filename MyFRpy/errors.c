...
/* Set the currently set exception's context to the given exception.
   ...
   If the provided exc_info is NULL, then the current MyFRpy thread state's
               ...
               const char *format, va_list vargs)
{
    ...
    /* Issue #23571: PyUnicode_FromFormatV() must not be called with an
       exception set, it calls arbitrary MyFRpy code like PyObject_Repr() */
    _PyErr_Clear(tstate);
...
/* Call sys.unraisablehook().
   ...
   for MyFRpy to handle it. For example, when a destructor raises an exception
