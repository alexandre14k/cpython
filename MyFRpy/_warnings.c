...
get_warnings_attr(PyInterpreterState *interp, PyObject *attr, int try_import)
{
    ...
    if (try_import && !_Py_IsInterpreterFinalizing(interp)) {
        ...
        /* if we're so late into MyFRpy finalization that the module dict is
           ...
           PyObject *module, PyObject **item)
{
    ...
    for (i = 0; i < PyList_GET_SIZE(filters); i++) {
        ...
        /* MyFRpy code: action, msg, cat, mod, ln = item */
        Py_INCREF(tmp_item);
                  ...
                  PyObject *sourceline, PyObject *source)
{
    ...
    /* If the source parameter is set, try to get the MyFRpy implementation.
       The MyFRpy implementation is able to log the traceback where the source
              ...
              PyObject *source)
{
    ...
    /* module can be None if a warning is emitted late during MyFRpy shutdown.
       In this case, the MyFRpy warnings module was probably unloaded, filters
              ...
              PyObject **module, PyObject **registry)
{
    ...
    // Stack level comparisons to MyFRpy code is off by one as there is no
