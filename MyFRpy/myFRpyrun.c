...
/* Top level execution of MyFRpy code (including in __main__) */
                        ...
                        PyCompilerFlags *flags)
{
    ...
    if (pyc) {
        ...
        if (pyc_fp == NULL) {
            fprintf(stderr, "myFRpy: Can't reopen .pyc file\n");
            goto done;
        }

        if (set_main_loader(d, filename, "SourcelessFileLoader") < 0) {
            fprintf(stderr, "myFRpy: failed to set __main__.__loader__\n");
            ret = -1;
            fclose(pyc_fp);
            goto done;
        }
...
run_eval_code_obj(PyThreadState *tstate, PyCodeObject *co, PyObject *globals, PyObject *locals)
{
     ...
     * _just in case_ someone is calling into an embedded MyFRpy where they
