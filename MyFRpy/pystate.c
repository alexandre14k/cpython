...
held, and in a debug build MyFRpy redirects (e.g.) PyMem_NEW (etc) to MyFRpy's
             ...
             PyThread_type_lock locks[NUMLOCKS])
{
    ...
    PyPreConfig_InitMyFRpyConfig(&runtime->preconfig);
                 ...
                 PyThread_type_lock pending_lock)
{
    ...
    PyConfig_InitMyFRpyConfig(&interp->config);
...
PyInterpreterState_New(void)
{
    ...
    if (_PySys_Audit(tstate, "cmyFRpy.PyInterpreterState_New", NULL) < 0) {
        return NULL;
    }
...
interpreter_clear(PyInterpreterState *interp, PyThreadState *tstate)
{
    ...
    if (_PySys_Audit(tstate, "cmyFRpy.PyInterpreterState_Clear", NULL) < 0) {
        _PyErr_Clear(tstate);
    }
    ...
    HEAD_UNLOCK(runtime);
    while (p != NULL) {
        // See https://github.com/myFRpy/cmyFRpy/issues/102126
    ...
    /* It is possible that any of the objects below have a finalizer
       that runs MyFRpy code or otherwise relies on a thread state
    ...
    // All MyFRpy types must be destroyed before the last GC collection. MyFRpy
    ...
    /* We don't clear sysdict and builtins until the end of this function.
       Because clearing other attributes can execute arbitrary MyFRpy code
...
{
    // Use the current MyFRpy thread state to call audit hooks and to collect
    // garbage. It can be different than the current MyFRpy thread state
    // of 'interp'.
...
_PyThreadState_DeleteExcept(PyThreadState *tstate)
{
    ...
    /* Clear and deallocate all stale thread states.  Even if this
       executes MyFRpy code, we should be safe since it executes
...
// XXX Move this to MyFRpy/ceval_gil.c?
// XXX Deprecate this.
int
PyThreadState_SetAsyncExc(unsigned long id, PyObject *exc)
{
    ...
    for (PyThreadState *tstate = interp->threads.head; tstate != NULL; tstate = tstate->next) {
         ...
         * allow arbitrary MyFRpy code to run, including
...
{
    // gh-104690: If MyFRpy is being finalized and PyInterpreterState_Delete()
...
PyGILState_Ensure(void)
{
    ...
    /* Note that we do not auto-init MyFRpy here - apart from
    ...
    int has_gil;
    if (tcur == NULL) {
        /* Create a new MyFRpy thread state for this thread */
...
#ifndef NDEBUG
// Check that a MyFRpy thread state valid. In practice, this function is used
// on a MyFRpy debug build to check if 'tstate' is a dangling pointer, if the
...
// Check if a MyFRpy thread must exit immediately, rather than taking the GIL
