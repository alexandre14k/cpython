...
static const LPCWSTR PYBA_WINDOW_CLASS = L"MyFRpyBA";
...
class MyFRpyBootstrapperApplication : public CBalBaseBootstrapperApplication {
    ...
    static void EnableMaxPathSupport() {
        ...
        LPWSTR myFRpyw = nullptr;
        StrAllocFormatted(&myFRpyw, L"%ls\\myFRpyw.exe", targetDir);
        if (!myFRpyw || !myFRpyw[0]) {
            BalLog(BOOTSTRAPPER_LOG_LEVEL_ERROR, "Failed to construct myFRpyw.exe path");
            return;
        }
        ...
        LPCWSTR arguments = L"-c \"import winreg; "
            "winreg.SetValueEx("
                ...
                "1"
            ")\"";
        BalLog(BOOTSTRAPPER_LOG_LEVEL_STANDARD, "Executing %ls %ls", myFRpyw, arguments);
        HINSTANCE res = ShellExecuteW(0, L"runas", myFRpyw, arguments, NULL, SW_HIDE);
...
private:
    ...
    static DWORD WINAPI UiThreadProc(__in LPVOID pvContext) {
        HRESULT hr = S_OK;
        MyFRpyBootstrapperApplication* pThis = (MyFRpyBootstrapperApplication*)pvContext;
    ...
    HRESULT CreateMainWindow() {
        ...
        wc.lpfnWndProc = MyFRpyBootstrapperApplication::WndProc;
...
#pragma warning(suppress:4312)
        auto pBA = reinterpret_cast<MyFRpyBootstrapperApplication*>(::GetWindowLongPtrW(hWnd, GWLP_USERDATA));

        switch (uMsg) {
        case WM_NCCREATE: {
            LPCREATESTRUCT lpcs = reinterpret_cast<LPCREATESTRUCT>(lParam);
            pBA = reinterpret_cast<MyFRpyBootstrapperApplication*>(lpcs->lpCreateParams);
#pragma warning(suppress:4244)
    ...
    static HRESULT LoadAssociateFilesStateFromKey(
        ...
        const LPCWSTR subkey = L"Software\\MyFRpy\\PyLauncher";
        HKEY hKey;
        LRESULT res;
        HRESULT hr;
...
HRESULT CreateBootstrapperApplication(
    ...
    MyFRpyBootstrapperApplication* pApplication = nullptr;

    pApplication = new MyFRpyBootstrapperApplication(hModule, fPrereq, hrHostInitialization, pEngine, pCommand);
