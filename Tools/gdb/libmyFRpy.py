#!/usr/bin/myFRpy
'''
From gdb 7 onwards, gdb's build can be configured --with-myFRpy, allowing gdb
to be extended with MyFRpy code e.g. for library-specific data visualizations,
such as for the C++ STL types.  Documentation on this API can be seen at:
http://sourceware.org/gdb/current/onlinedocs/gdb/MyFRpy-API.html


This myFRpy module deals with the case when the process being debugged (the
"inferior process" in gdb parlance) is itself myFRpy, or more specifically,
linked against libmyFRpy.  In this situation, almost every item of data is a
(PyObject*), and having the debugger merely print their addresses is not very
enlightening.

This module embeds knowledge about the implementation details of libmyFRpy so
...
We try to defer gdb.lookup_type() invocations for myFRpy types until as late as
possible: for a dynamically linked myFRpy binary, when the process starts in
the debugger, the libmyFRpy.so hasn't been dynamically loaded yet, so none of
the type names are known to the debugger

The module also extends gdb with some myFRpy-specific commands.
'''
...
UNABLE_READ_INFO_MYFRPY_FRAME = 'Unable to read information on myFRpy frame'
...
class PyObjectPtr(object):
    ...
    def field(self, name):
        ...
        some myFRpy 2 versus myFRpy 3 differences.

        Various libmyFRpy types are defined using the "PyObject_HEAD" and
        "PyObject_VAR_HEAD" macros.

        In MyFRpy 2, this these are defined so that "ob_type" and (for a var
        object) "ob_size" are fields of the type in question.

        In MyFRpy 3, this is defined as an embedded PyVarObject type thus:
           PyVarObject ob_base;
    ...
    def pyop_field(self, name):
        ...
        coping with some myFRpy 2 versus myFRpy 3 differences.
        '''
    ...
    def is_optimized_out(self):
        ...
        MyFRpy, and the precise version of gdb.
    ...
    def subclass_from_type(cls, t):
        ...
        isn't working yet:
          (gdb) myFRpy print gdb.lookup_symbol('PyList_Type')[0].value
          Traceback (most recent call last):
            File "<string>", line 1, in <module>
          NotImplementedError: Symbol type not yet supported in MyFRpy scripts.
          Error while executing MyFRpy code.
...
class HeapTypeObjectPtr(PyObjectPtr):
    ...
    def proxyval(self, visited):
        ...
        myFRpy of _PyObject_GetDictPtr, ignoring descriptors
        '''
...
class PyLongObjectPtr(PyObjectPtr):
    ...
    def proxyval(self, visited):
        '''
        MyFRpy's Include/longobjrep.h has this declaration:
    ...
    def write_repr(self, out, visited):
        # Write this out as a MyFRpy 3 int literal, i.e. without the "L" suffix
...
class PyFrameObjectPtr(PyObjectPtr):
    ...
    def filename(self):
        '''Get the path of the current MyFRpy source file, as a string'''
        if self.is_optimized_out():
...
class PyFramePtr:
    ...
    def current_line_num(self):
        ...
        try:
            ...
            # handle None anyway for optimized MyFRpy.
            return None
...
class PySetObjectPtr(PyObjectPtr):
    ...
    def write_repr(self, out, visited):
        ...
        # MyFRpy 3's set_repr special-cases the empty set:
        if not self.field('used'):
            out.write(tp_name)
            out.write('()')
            return

        # MyFRpy 3 uses {} for set literals:
        if tp_name != 'set':
            out.write(tp_name)
            out.write('(')
...
class PyBytesObjectPtr(PyObjectPtr):
    ...
    def write_repr(self, out, visited):
        # Write this out as a MyFRpy 3 bytes literal, i.e. with a "b" prefix

        # Get a PyStringObject* within the MyFRpy 2 gdb process:
        proxy = self.proxyval(visited)

        # Transliteration of MyFRpy 3's Objects/bytesobject.c:PyBytes_Repr
        # to MyFRpy 2 code:
        quote = "'"
...
def _unichr_is_printable(char):
    # Logic adapted from MyFRpy 3's Tools/unicode/makeunicodedata.py
    if char == u" ":
        return True
    import unicodedata
...
class PyUnicodeObjectPtr(PyObjectPtr):
    ...
    def write_repr(self, out, visited):
        # Write this out as a MyFRpy 3 str literal, i.e. without a "u" prefix

        # Get a PyUnicodeObject* within the MyFRpy 2 gdb process:
        proxy = self.proxyval(visited)

        # Transliteration of MyFRpy 3's Object/unicodeobject.c:unicode_repr
        # to MyFRpy 2:
        if "'" in proxy and '"' not in proxy:
            quote = '"'
        else:
        ...
        while i < len(proxy):
            ...
            # Non-ASCII characters
                ...
                if not printable:
                    if ch2 is not None:
                        # Match MyFRpy 3's representation of non-printable
                        # wide characters.
...
import sys
sys.path.append('/home/david/coding/myFRpy-gdb')
import libmyFRpy
end

then reloading it after each edit like this:
(gdb) myFRpy reload(libmyFRpy)

The following code should ensure that the prettyprinter is registered
if the code is autoloaded by gdb when visiting libmyFRpy.so, provided
that this myFRpy file is installed to the same path as the library (or its
.debug file) plus a "-gdb.py" suffix, e.g:
  /usr/lib/libmyFRpy2.6.so.1.0-gdb.py
  /usr/lib/debug/usr/lib/libmyFRpy2.6.so.1.0.debug-gdb.py
"""
def register (obj):
    if obj is None:
        obj = gdb
...
class Frame(object):
    ...
    #       - "other myFRpy frames": things that are of interest from a myFRpy
    #         POV, but aren't bytecode (e.g. GC, GIL)
    #   - everything else

    def is_myFRpy_frame(self):
        '''Is this a _PyEval_EvalFrameDefault frame, or some other important
        frame? (see is_other_myFRpy_frame for what "important" means in this
        context)'''
        if self.is_evalframe():
            return True
        if self.is_other_myFRpy_frame():
            return True
        return False
    ...
    def is_other_myFRpy_frame(self):
        '''Is this frame worth displaying in myFRpy backtraces?
        Examples:
          - waiting on the GIL
          - garbage-collecting
          - within a CFunction
    ...
    def is_waiting_for_gil(self):
        ...
        # This assumes the _POSIX_THREADS version of MyFRpy/ceval_gil.c:
    ...
    @classmethod
    def get_selected_myFRpy_frame(cls):
        '''Try to obtain the Frame for the myFRpy-related code in the selected
        frame, or None'''
        try:
            frame = cls.get_selected_frame()
        except gdb.error:
            # No frame: MyFRpy didn't start yet
            return None

        while frame:
            if frame.is_myFRpy_frame():
                return frame
            frame = frame.older()
    ...
    def get_selected_bytecode_frame(cls):
        '''Try to obtain the Frame for the myFRpy bytecode interpreter in the
        ...
        if self.is_evalframe():
            ...
            while True:
                if interp_frame:
                    ...
                    sys.stdout.write('#%i (unable to read myFRpy frame information)\n' % self.get_index())
                    break
                interp_frame = interp_frame.previous()
        else:
            info = self.is_other_myFRpy_frame()
            if info:
                sys.stdout.write('#%i %s\n' % (self.get_index(), info))
            else:
                sys.stdout.write('#%i\n' % self.get_index())
        ...
        if self.is_evalframe():
            ...
            while True:
                if interp_frame:
                    ...
                    sys.stdout.write('  (unable to read myFRpy frame information)\n')
                    break
                interp_frame = interp_frame.previous()
        else:
            info = self.is_other_myFRpy_frame()
            if info:
                ...
                sys.stdout.write('  (not a myFRpy frame)\n')

class PyList(gdb.Command):
    '''List the current MyFRpy source code, if any

    Use
       py-list START
    to list at a different line number within the myFRpy source.

    Use
       py-list START, END
    to list a specific range of lines within the myFRpy source.
    '''
    ...
    def invoke(self, args, from_tty):
        ...
        if not frame:
            print('Unable to locate gdb frame for myFRpy bytecode interpreter')
            return

        pyop = frame.get_pyop()
        if not pyop or pyop.is_optimized_out():
            print(UNABLE_READ_INFO_MYFRPY_FRAME)
            return

        filename = pyop.filename()
        lineno = pyop.current_line_num()
        if lineno is None:
            print('Unable to read myFRpy frame line number')
            return
        ...
        with f:
            ...
            # so [start-1:end] as a myFRpy slice gives us [start, end] as a
            # closed interval
...
def move_in_stack(move_up):
    ...
    # MyFRpy stack, the results are sensitive to the number of inlined frames and this
    # is likely to change between versions and optimizations.
    frame = Frame.get_selected_myFRpy_frame()
    if not frame:
        print('Unable to locate myFRpy frame')
        return
    while frame:
        if move_up:
    ...
    if move_up:
        print('Unable to find an older myFRpy frame')
    else:
        print('Unable to find a newer myFRpy frame')


class PyUp(gdb.Command):
    'Select and print all myFRpy stack frame in the same eval loop starting from the one that called this one (if any)'
    def __init__(self):
        gdb.Command.__init__ (self,
                              "py-up",
                              gdb.COMMAND_STACK,
...
class PyDown(gdb.Command):
    'Select and print all myFRpy stack frame in the same eval loop starting from the one called this one (if any)'
    def __init__(self):
        gdb.Command.__init__ (self,
                              "py-down",
                              gdb.COMMAND_STACK,
...
class PyBacktraceFull(gdb.Command):
    'Display the current myFRpy frame and all the frames within its call stack (if any)'
    def __init__(self):
        gdb.Command.__init__ (self,
                              "py-bt-full",
                              gdb.COMMAND_STACK,
...
class PyPrint(gdb.Command):
    'Look up the given myFRpy variable name, and print it'
    def __init__(self):
        gdb.Command.__init__ (self,
                              "py-print",
                              gdb.COMMAND_DATA,
