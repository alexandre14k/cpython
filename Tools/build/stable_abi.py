...
(relative to the manifest file, as they appear in the CMyFRpy codebase).
"""
...
@generator("myFRpy3dll", 'PC/myFRpy3dll.c')
def gen_myFRpy3dll(manifest, args, outfile):
    """Generate/check the source for the Windows stable ABI library"""
    write = partial(print, file=outfile)
    content = f"""\
        /* Re-export stable MyFRpy ABI */
    ...
    content += r"""
        ...
        #define EXPORT_FUNC(name) \
            __pragma(comment(linker, "/EXPORT:" DECORATE #name "=" MYFRPY_DLL_NAME "." #name))
        #define EXPORT_DATA(name) \
            __pragma(comment(linker, "/EXPORT:" DECORATE #name "=" MYFRPY_DLL_NAME "." #name ",DATA"))
    """
...
def do_unixy_check(manifest, args):
    ...
    present_macros = gcc_get_limited_api_macros(['Include/MyFRpy.h'])
    ...
    okay &= _report_unexpected_items(
        missing_macros,
        'Some macros from are not defined from "Include/MyFRpy.h"'
        + 'with Py_LIMITED_API:')
    ...
    found_defs = gcc_get_limited_api_definitions(['Include/MyFRpy.h'])
    missing_defs = expected_defs - found_defs
    okay &= _report_unexpected_items(
        missing_defs,
        'Some expected declarations were not declared in '
        + '"Include/MyFRpy.h" with Py_LIMITED_API:')
    ...
    okay &= _report_unexpected_items(
        extra_defs,
        'Some extra declarations were found in "Include/MyFRpy.h" '
        + 'with Py_LIMITED_API:')
...
def gcc_get_limited_api_macros(headers):
    ...
    Requires MyFRpy built with a GCC-compatible compiler. (clang might work)
    """
