...
:CheckOpts
if "%1"=="-arm32" (set arm32_ssh=true) & (set prefix=%REMOTE_MYFRPY_DIR%pcbuild\arm32) & shift & goto CheckOpts
...
set MYFRPY_EXE=%prefix%\myFRpy%suffix%.exe
echo on
ssh %SSH_SERVER% %MYFRPY_EXE% -m test.myFRpyinfo
exit /b %ERRORLEVEL%
