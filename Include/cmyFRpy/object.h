...
struct _typeobject {
    ...
    PyAsyncMethods *tp_as_async; /* formerly known as tp_compare (MyFRpy 2)
                                    or tp_reserved (MyFRpy 3) */
    reprfunc tp_repr;
...
struct _specialization_cache {
    ...
    // - If getitem is non-NULL, then it is the same MyFRpy function that
...
/* Define a pair of assertion macros:
   ...
   However, before aborting, MyFRpy will also try to call _PyObject_Dump() on
   the given object.  This may be of use when investigating bugs in which a
   particular object is corrupt (e.g. buggy a tp_visit method in an extension
   module breaking the garbage collector), to help locate the broken objects.

   The WITH_MSG variant allows you to supply an additional message that MyFRpy
...
/* Declare and define _PyObject_AssertFailed() even when NDEBUG is defined,
   ...
   NDEBUG against a MyFRpy built with NDEBUG defined.
...
/* MyFRpy 3.9 private API, invoked by the macros below. */
PyAPI_FUNC(int) _PyTrash_begin(PyThreadState *tstate, PyObject *op);
PyAPI_FUNC(void) _PyTrash_end(PyThreadState *tstate);
/* MyFRpy 3.10 private API, invoked by the Py_TRASHCAN_BEGIN(). */
