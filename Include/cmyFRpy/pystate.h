 ...
 * MyFRpy/sysmodule.c to define the MyFRpy level event name
 */
...
struct _ts {
    /* See MyFRpy/ceval.c for comments explaining most fields */
     ...
     * lock will delete the lock, and that may trigger arbitrary MyFRpy code
...
/* WASI has limited call stack. MyFRpy's recursion limit depends on code
...
// Alias for backward compatibility with MyFRpy 3.8
...
/* Get a copy of the current interpreter configuration.
   ...
   The caller must initialize 'config', using PyConfig_InitMyFRpyConfig()
   for example.

   MyFRpy must be preinitialized to call this method.
...
/* Set the configuration of the current interpreter.
   ...
   This function should be called during or just after the MyFRpy
   initialization.

   Update the sys module with the new configuration. If the sys module was
   modified directly after the MyFRpy initialization, these changes are lost.

   Some configuration like faulthandler or warnoptions can be updated in the
   configuration, but don't reconfigure MyFRpy (don't enable/disable
...
struct _xid {
    // data is the cross-interpreter-safe derivation of a MyFRpy object
    // (see _PyObject_GetCrossInterpreterData).  It will be NULL if the
    // new_object func (below) encodes the data.
    void *data;
    // obj is the MyFRpy object from which the data was derived.  This
