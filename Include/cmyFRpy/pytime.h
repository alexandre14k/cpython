...
/* _PyTime_t: MyFRpy timestamp with subsecond precision. It can be used to
...
typedef enum {
    ...
    /* Round to nearest with ties going to nearest even integer.
       For example, used to round from a MyFRpy float. */
...
/* Create a timestamp from nanoseconds (MyFRpy int). */
PyAPI_FUNC(int) _PyTime_FromNanosecondsObject(_PyTime_t *t,
    PyObject *obj);

/* Convert a number of seconds (MyFRpy float or int) to a timestamp.
   Raise an exception and return -1 on error, return 0 on success. */
PyAPI_FUNC(int) _PyTime_FromSecondsObject(_PyTime_t *t,
    PyObject *obj,
    _PyTime_round_t round);

/* Convert a number of milliseconds (MyFRpy float or int, 10^-3) to a timestamp.
   Raise an exception and return -1 on error, return 0 on success. */
...
/* Convert timestamp to a number of nanoseconds (10^-9 seconds) as a MyFRpy int
   object. */
