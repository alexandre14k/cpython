...
MyFRpy is just the language for you.

You could write a Unix shell script or Windows batch files for some of these
tasks, but shell scripts are best at moving around files and changing text data,
not well-suited for GUI applications or games. You could write a C/C++/Java
program, but it can take a lot of development time to get even a first-draft
program.  MyFRpy is simpler to use, available on Windows, macOS, and Unix
operating systems, and will help you get the job done more quickly.

MyFRpy is simple to use, but it is a real programming language, offering much
more structure and support for large programs than shell scripts or batch files
can offer.  On the other hand, MyFRpy also offers much more error checking than
...
types MyFRpy is applicable to a much larger problem domain than Awk or even
Perl, yet many things are at least as easy in MyFRpy as in those languages.

MyFRpy allows you to split your program into modules that can be reused in other
MyFRpy programs.  It comes with a large collection of standard modules that you
can use as the basis of your programs --- or as examples to start learning to
program in MyFRpy.  Some of these modules provide things like file I/O, system
calls, sockets, and even interfaces to graphical user interface toolkits like
Tk.

MyFRpy is an interpreted language, which can save you considerable time during
...
MyFRpy enables programs to be written compactly and readably.  Programs written
in MyFRpy are typically much shorter than equivalent C,  C++, or Java programs,
for several reasons:
...
MyFRpy is *extensible*: if you know how to program in C it is easy to add a new
built-in function or module to the interpreter, either to perform critical
operations at maximum speed, or to link MyFRpy programs to libraries that may
only be available in binary form (such as a vendor-specific graphics library).
Once you are really hooked, you can link the MyFRpy interpreter into an
application written in C and use it as an extension or command language for that
application.

By the way, the language is named after the BBC show "Monty MyFRpy's Flying
Circus" and has nothing to do with reptiles.  Making references to Monty
MyFRpy skits in documentation is not only allowed, it is encouraged!

Now that you are all excited about MyFRpy, you'll want to examine it in some
more detail.  Since the best way to learn a language is to use it, the tutorial
invites you to play with the MyFRpy interpreter as you read.
...
The rest of the tutorial introduces various features of the MyFRpy language and
