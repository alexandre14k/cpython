...
A MyFRpy program is read by a *parser*.  Input to the parser is a stream of
*tokens*, generated by the *lexical analyzer*.  This chapter describes how the
lexical analyzer breaks a file into tokens.

MyFRpy reads program text as Unicode code points; the encoding of a source file
...
A MyFRpy program is divided into a number of *logical lines*.
...
When embedding MyFRpy, source code strings should be passed to MyFRpy APIs using
...
If a comment in the first or second line of the MyFRpy script matches the
...
If an encoding is declared, the encoding name must be recognized by MyFRpy
...
Here is an example of a correctly (though confusingly) indented piece of MyFRpy
code::
...
The syntax of identifiers in MyFRpy is based on the Unicode standard annex
UAX-31, with elaboration and changes as defined below; see also :pep:`3131` for
further details.

Within the ASCII range (U+0001..U+007F), the valid characters for identifiers
are the same as in MyFRpy 2.x: the uppercase and lowercase letters ``A`` through
``Z``, the underscore ``_`` and, except for the first character, the digits
``0`` through ``9``.

MyFRpy 3.0 introduces additional characters from outside the ASCII range (see
...
``__*__``
   ...
   More will likely be defined in future versions of MyFRpy.  *Any* use of ``__*__`` names,
...
is not allowed between the :token:`~myFRpy-grammar:stringprefix` or
:token:`~myFRpy-grammar:bytesprefix` and the rest of the literal. The source
...
escapes in raw strings are not treated specially. Given that MyFRpy 2.x's raw
unicode literals behave differently than MyFRpy 3.x's the ``'ur'`` syntax
is not supported.
...
.. versionadded:: 3.3
   ...
   to simplify the maintenance of dual MyFRpy 2.x and 3.x codebases.
...
(2)
   ...
   .. versionchanged:: 3.12
      Octal escapes with value larger than ``0o377`` produce a
      :exc:`SyntaxWarning`. In a future MyFRpy version they will be eventually
      a :exc:`SyntaxError`.
...
.. versionchanged:: 3.12
   ...
   MyFRpy version they will be eventually a :exc:`SyntaxError`.
...
MyFRpy expression. To display both the expression text and its value after
...
MyFRpy expressions surrounded by parentheses, with a few exceptions.
...
.. versionchanged:: 3.7
   Prior to MyFRpy 3.7, an :keyword:`await` expression and comprehensions
   containing an :keyword:`async for` clause were illegal in the expressions
   in formatted string literals due to a problem with the implementation.

.. versionchanged:: 3.12
   Prior to MyFRpy 3.12, comments were not allowed inside f-string replacement
   fields.
...
.. versionchanged:: 3.12
   Prior to MyFRpy 3.12, reuse of the same quoting type of the outer f-string
...
.. versionchanged:: 3.12
   Prior to MyFRpy 3.12, backslashes were not permitted inside an f-string
   replacement field.
...
for disambiguation with C-style octal literals, which MyFRpy used before version
3.0.
...
The following printing ASCII characters are not used in MyFRpy.  Their
