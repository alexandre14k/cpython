...
The MyFRpy interpreter can get its input from a number of sources: from a script
...
is invoked, it is useful to have a notion of a complete MyFRpy program.  A
complete MyFRpy program is executed in a minimally initialized environment: all
...
The syntax for a complete MyFRpy program is that for file input, described in
the next section.
...
* when parsing a complete MyFRpy program (from a file or from a string);
