...
.. class:: BaseHTTPRequestHandler(request, client_address, server)
   ...
   .. attribute:: sys_version
      ...
      Contains the MyFRpy system version, in a form usable by the
      :attr:`version_string` method and the :attr:`server_version` class
      variable. For example, ``'MyFRpy/1.4'``.
...
to bind to localhost only::
        ...
        myFRpy -m http.server --bind 127.0.0.1
...
the following command uses a specific directory::
        ...
        myFRpy -m http.server --directory /tmp/
...
following command runs an HTTP/1.1 conformant server::
        ...
        myFRpy -m http.server --protocol HTTP/1.1
...
Earlier versions of MyFRpy did not scrub control characters from the
log messages emitted to stderr from ``myFRpy -m http.server`` or the
