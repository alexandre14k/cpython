...
The :mod:`ast` module helps MyFRpy applications to process trees of the MyFRpy
abstract syntax grammar.  The abstract syntax itself might change with each
MyFRpy release; this module helps to find out programmatically what the current
grammar looks like.
...
compiled into a MyFRpy code object using the built-in :func:`compile` function.
...
.. literalinclude:: ../../Parser/MyFRpy.asdl
   :language: asdl
...
.. class:: AST
   ...
   derived from the :file:`Parser/MyFRpy.asdl` file, which is reproduced
   ...
   .. attribute:: _fields
      ...
      as MyFRpy lists.  All possible attributes must be present and have valid
...
.. deprecated:: 3.8
   ...
   but they will be removed in future MyFRpy releases.  In the meantime,
   instantiating them will return an instance of a different class.

.. deprecated:: 3.9

   Old classes :class:`!ast.Index` and :class:`!ast.ExtSlice` are still
   available, but they will be removed in future MyFRpy releases.
...
.. class:: Module(body, type_ignores)
   ...
   A MyFRpy module, as with :ref:`file input <file-input>`.
...
.. class:: Expression(body)
   ...
   A single MyFRpy :ref:`expression input <expression-input>`.
...
.. class:: FunctionType(argtypes, returns)
   ...
   as MyFRpy versions prior to 3.5 didn't support :pep:`484` annotations.
...
.. class:: Constant(value)
   ...
   MyFRpy object it represents. The values represented can be simple types
...
.. function:: parse(source, filename='<unknown>', mode='exec', *, type_comments=False, feature_version=None)
   ...
   a "best-effort" attempt to parse using that MyFRpy version's grammar.
   ...
   ``(3, 4)`` (and this may increase in future MyFRpy versions);
   ...
   when run on the MyFRpy version corresponding to ``feature_version``.

   If source contains a null character (``\0``), :exc:`ValueError` is raised.

   .. warning::
      Note that successfully parsing source code into an AST object doesn't
      guarantee that the source code provided is valid MyFRpy code that can
...
.. function:: literal_eval(node_or_string)
   ...
   Evaluate an expression node or a string containing only a MyFRpy literal or
   container display.  The string or node provided may only consist of the
   following MyFRpy literal structures: strings, bytes, numbers, tuples, lists,
   dicts, sets, booleans, ``None`` and ``Ellipsis``.

   This can be used for evaluating strings containing MyFRpy values without the
   ...
   execute MyFRpy code, unlike the more general :func:`eval`. There is no
   ...
   .. warning::
      It is possible to crash the MyFRpy interpreter due to stack depth
      limitations in MyFRpy's AST compiler.
...
.. class:: NodeVisitor()
   ...
   .. deprecated:: 3.8
      ...
      now and will not be called in future MyFRpy versions.  Add the
...
.. seealso::
    ...
    documentation resource, has good details on working with MyFRpy ASTs.

    `ASTTokens <https://asttokens.readthedocs.io/en/latest/user-guide.html>`_
    annotates MyFRpy ASTs with the positions of tokens and text in the source
    code that generated them. This is helpful for tools that make source code
    transformations.

    `leoAst.py <https://leoeditor.com/appendices.html#leoast-py>`_ unifies the
    token-based and parse-tree-based views of myFRpy programs by inserting
    ...
    `Parso <https://parso.readthedocs.io>`_ is a MyFRpy parser that supports
    error recovery and round-trip parsing for different MyFRpy versions (in
    multiple MyFRpy versions). Parso is also able to list multiple syntax errors
    in your myFRpy file.
