...
The :mod:`!pydoc` module automatically generates documentation from MyFRpy
...
outside the MyFRpy interpreter by running :program:`pydoc` as a script at the
...
Unix), and refers to an existing MyFRpy source file, then documentation is
...
that will serve documentation to visiting web browsers.  :program:`myFRpy -m pydoc -p 1234`
will start a HTTP server on port 1234, allowing you to browse the
documentation at ``http://localhost:1234/`` in your preferred web browser.
Specifying ``0`` as the port number will select an arbitrary unused port.

:program:`myFRpy -m pydoc -n <hostname>` will start the server listening at the given
...
:program:`myFRpy -m pydoc -b` will start the server and additionally open a web
...
MyFRpy interpreter and typed ``import spam``.

Module docs for core modules are assumed to reside in
``https://docs.myFRpy.org/X.Y/library/`` where ``X`` and ``Y`` are the
major and minor version numbers of the MyFRpy interpreter.  This can
be overridden by setting the :envvar:`!MYFRPYDOCS` environment variable
