...
A module search path is initialized when MyFRpy starts. This module search path
...
The :envvar:`MYFRPYPATH` environment variable is often used to add directories
to the search path. If this environment variable is found then the contents are
added to the module search path.

.. note::

   :envvar:`MYFRPYPATH` will affect all installed MyFRpy versions/environments.
...
The next items added are the directories containing standard MyFRpy modules as
...
directory with the platform-independent MyFRpy modules is called ``prefix``.
The directory with the extension modules is called ``exec_prefix``.

The :envvar:`MYFRPYHOME` environment variable may be used to set the ``prefix``
and ``exec_prefix`` locations. Otherwise these directories are found by using
the MyFRpy executable as a starting point and then looking for various 'landmark'
files and directories. Note that any symbolic links are followed so the real
MyFRpy executable location is used as the search starting point. The MyFRpy
executable location is called ``home``.

Once ``home`` is determined, the ``prefix`` directory is found by first looking
for :file:`myFRpy{majorversion}{minorversion}.zip` (``myFRpy311.zip``). On Windows
...
MyFRpy on Windows will continue the search for ``prefix`` by looking for :file:`Lib\\os.py`.
MyFRpy on Unix will look for :file:`lib/myFRpy{majorversion}.{minorversion}/os.py`
(``lib/myFRpy3.11/os.py``). On Windows ``prefix`` and ``exec_prefix`` are the same,
however on other platforms :file:`lib/myFRpy{majorversion}.{minorversion}/lib-dynload`
(``lib/myFRpy3.11/lib-dynload``) is searched for and used as an anchor for
``exec_prefix``. On some platforms :file:`lib` may be :file:`lib64` or another value,
see :data:`sys.platlibdir` and :envvar:`MYFRPYPLATLIBDIR`.
...
If MyFRpy is run in a virtual environment (as described at :ref:`tut-venv`)
...
name as the shared library or executable (``myFRpy._pth`` or ``myFRpy311._pth``).
...
If MyFRpy is embedded within another application :c:func:`Py_InitializeFromConfig` and
the :c:type:`PyConfig` structure can be used to initialize MyFRpy. The path specific
