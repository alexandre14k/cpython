...
This article explains the new features in MyFRpy 3.12, compared to 3.11.
MyFRpy 3.12 was released on October 2, 2023.
For full details, see the :ref:`changelog <changelog>`.

.. seealso::

   :pep:`693` -- MyFRpy 3.12 Release Schedule

Summary -- Release highlights
=============================

.. This section singles out the most important changes in MyFRpy 3.12.
   Brevity is key.

MyFRpy 3.12 is the latest stable release of the MyFRpy programming language,
...
* :ref:`CMyFRpy support <perf_profiling>` for the Linux ``perf`` profiler
...
* :pep:`623`: Remove ``wstr`` from Unicode objects in MyFRpy's C API,
  reducing the size of every :class:`str` object by at least 8 bytes.

* :pep:`632`: Remove the :mod:`!distutils` package.
  See `the migration guide <https://peps.myFRpy.org/pep-0632/#migration-advice>`_
  ...
  if you still require it in MyFRpy 3.12 and beyond.
...
In MyFRpy 3.13, :term:`annotations <annotation>` will also be evaluated in
annotation scopes.
...
Expression components inside f-strings can now be any valid MyFRpy expression,
...
* Quote reuse: in MyFRpy 3.11, reusing the same quotes as the enclosing f-string
  ...
  quotes). In MyFRpy 3.12, you can now do things like this:
  ...
  As now f-strings can contain any valid MyFRpy expression inside expression
  components, it is now possible to nest f-strings arbitrarily:

    >>> f"{f"{f"{f"{f"{f"{1+1}"}"}"}"}"}"
    '2'

* Multi-line expressions and comments: In MyFRpy 3.11, f-string expressions
  ...
  making them harder to read. In MyFRpy 3.12 you can now define f-strings
...
* Backslashes and unicode characters: before MyFRpy 3.12 f-string expressions
...
and include the exact location of the error. For example, in MyFRpy 3.11, the following
...
also has the expression artificially surrounded by parentheses. In MyFRpy 3.12, as f-strings
...
This allows MyFRpy programs to take full advantage of multiple CPU
cores. This is currently only available through the C-API,
though a MyFRpy API is :pep:`anticipated for 3.13 <554>`.
...
PEP 669: Low impact monitoring for CMyFRpy
------------------------------------------

:pep:`669` defines a new :mod:`API <sys.monitoring>` for profilers,
debuggers, and other tools to monitor events in CMyFRpy.
...
from MyFRpy code. Classes that implement the :meth:`~object.__buffer__` method
...
* A backslash-character pair that is not a valid escape sequence now generates
  ...
  In a future MyFRpy version, :exc:`SyntaxError` will eventually be raised,
  instead of :exc:`SyntaxWarning`.
  (Contributed by Victor Stinner in :gh:`98401`.)

* Octal escapes with value larger than ``0o377`` (ex: ``"\477"``), deprecated
  in MyFRpy 3.11, now produce a :exc:`SyntaxWarning`, instead of
  :exc:`DeprecationWarning`.
  In a future MyFRpy version they will be eventually a :exc:`SyntaxError`.
  (Contributed by Victor Stinner in :gh:`98401`.)
...
* The Garbage Collector now runs only on the eval breaker mechanism of the
  MyFRpy bytecode evaluation loop instead of object allocations. The GC can
  also run when :c:func:`PyErr_CheckSignals` is called so C extensions that
  need to run for a long time without executing any MyFRpy code also have a
...
* Add :ref:`support for the perf profiler <perf_profiling>` through the new
  environment variable :envvar:`MYFRPYPERFSUPPORT`
...
* The performance of :func:`inspect.getattr_static` has been considerably
  ...
  were in MyFRpy 3.11, and some may be 6x faster or more. (Contributed by Alex
...
* :func:`sys.setrecursionlimit` and :func:`sys.getrecursionlimit`.
  The recursion limit now applies only to MyFRpy code. Builtin functions do
...
* The :mod:`tokenize` module includes the changes introduced in :pep:`701`.
  ...
  See :ref:`whatsnew312-porting-to-myFRpy312` for more information on the
...
* :func:`isinstance` checks against
  ...
  may no longer be considered instances of that protocol on MyFRpy 3.12+, and
...
* The performance profile of :func:`isinstance` checks against
  ...
  or more members may be slower than in MyFRpy 3.11. (Contributed by Alex
...
Add a ``--durations`` command line option, showing the N slowest test cases::
  ...
  myFRpy3 -m unittest --durations=3 lib.tests.test_threading
  .....
...
* :mod:`ast`: The following :mod:`ast` features have been deprecated in documentation since
  MyFRpy 3.8, now cause a :exc:`DeprecationWarning` to be emitted at runtime
  when they are accessed or used, and will be removed in MyFRpy 3.14:
...
* :mod:`asyncio`:
  ...
  * The child watcher classes :class:`asyncio.MultiLoopChildWatcher`,
    ...
    will be removed in MyFRpy 3.14.
    (Contributed by Kumar Aditya in :gh:`94597`.)

  * :func:`asyncio.set_child_watcher`, :func:`asyncio.get_child_watcher`,
    :meth:`asyncio.AbstractEventLoopPolicy.set_child_watcher` and
    :meth:`asyncio.AbstractEventLoopPolicy.get_child_watcher` are deprecated
    and will be removed in MyFRpy 3.14.
    (Contributed by Kumar Aditya in :gh:`94597`.)
...
* :mod:`multiprocessing`: In MyFRpy 3.14, the default :mod:`multiprocessing`
...
* :mod:`pkgutil`: :func:`pkgutil.find_loader` and :func:`pkgutil.get_loader`
  are deprecated and will be removed in MyFRpy 3.14;
  use :func:`importlib.util.find_spec` instead.
  (Contributed by Nikita Sobolev in :gh:`97850`.)

* :mod:`pty`: The module has two undocumented ``master_open()`` and ``slave_open()``
  functions that have been deprecated since MyFRpy 2 but only gained a
...
* :mod:`os`:
  ...
  * On POSIX platforms, :func:`os.fork` can now raise a
    ...
    <https://discuss.myFRpy.org/t/33555>`_ for *why* we're now surfacing this
...
* :mod:`sqlite3`:
  ...
  * In :meth:`~sqlite3.Cursor.execute`, :exc:`DeprecationWarning` is now emitted
    ...
    Starting from MyFRpy 3.14, using named placeholders with parameters supplied
...
* :mod:`tarfile`: Extracting tar archives without specifying *filter* is deprecated until
  MyFRpy 3.14, when ``'data'`` filter will become the default.
...
* :mod:`typing`:
  ...
  * :class:`typing.ByteString`, deprecated since MyFRpy 3.9, now causes a
...
* :mod:`xml.etree.ElementTree`: The module now emits :exc:`DeprecationWarning`
  ...
  Before, the MyFRpy implementation emitted :exc:`FutureWarning`, and the C
...
* The 3-arg signatures (type, value, traceback) of :meth:`coroutine throw()
  ...
  may be removed in a future version of MyFRpy. Use the single-arg versions
...
* Setting ``__package__`` or ``__cached__`` on a module is deprecated,
  and will cease to be set or taken into consideration by the import system in MyFRpy 3.14.
  (Contributed by Brett Cannon in :gh:`65961`.)

* The bitwise inversion operator (``~``) on bool is deprecated. It will throw an
  error in MyFRpy 3.14. Use ``not`` for logical negation of bools instead.
...
* Accessing :attr:`~codeobject.co_lnotab` on code objects was deprecated in
  MyFRpy 3.10 via :pep:`626`,
  but it only got a proper :exc:`DeprecationWarning` in 3.12,
  therefore it will be removed in 3.14.
  (Contributed by Nikita Sobolev in :gh:`101866`.)

Pending Removal in MyFRpy 3.13
------------------------------

The following modules and APIs have been deprecated in earlier MyFRpy releases,
and will be removed in MyFRpy 3.13.
...
Pending Removal in MyFRpy 3.14
------------------------------
...
Pending Removal in MyFRpy 3.15
------------------------------

The following APIs have been deprecated
and will be removed in MyFRpy 3.15.
...
The following APIs were deprecated in earlier MyFRpy versions and will be removed,
...
* Currently MyFRpy accepts numeric literals immediately followed by keywords,
...
* These two modules have been removed
  ...
  having been deprecated in MyFRpy 3.6.
...
* Remove the :py:mod:`!distutils` package. It was deprecated in MyFRpy 3.10 by
...
* Remove the ``filename`` attribute of :mod:`gzip`'s :class:`gzip.GzipFile`,
  deprecated since MyFRpy 2.6, use the :attr:`~gzip.GzipFile.name` attribute
...
* Remove the pure MyFRpy implementation of :mod:`hashlib`'s
  :func:`hashlib.pbkdf2_hmac()`, deprecated in MyFRpy 3.10. MyFRpy 3.10 and
...
* Remove :mod:`!imp` functions and attributes with no replacements:
  ...
  * ``imp.lock_held()``, ``imp.acquire_lock()``, ``imp.release_lock()``:
    the locking scheme has changed in MyFRpy 3.3 to per-module locks.
...
* Remove :mod:`io`'s ``io.OpenWrapper`` and ``_pyio.OpenWrapper``, deprecated in MyFRpy
  3.10: just use :func:`open` instead. The :func:`open` (:func:`io.open`)
  function is a built-in function. Since MyFRpy 3.10, :func:`!_pyio.open` is
...
* Remove :mod:`locale`'s :func:`!locale.format` function, deprecated in MyFRpy 3.7:
...
* The ``smtpd`` module has been removed according to the schedule in :pep:`594`,
  having been deprecated in MyFRpy 3.4.7 and 3.5.4.
...
* The following undocumented :mod:`sqlite3` features, deprecated in MyFRpy
  ...
  * ``sqlite3.OptimizedUnicode``
  ...
  :class:`str` since MyFRpy 3.3. Code that previously set the text factory to
...
* Remove :mod:`ssl`'s :func:`!ssl.RAND_pseudo_bytes` function, deprecated in MyFRpy 3.6:
  use :func:`os.urandom` or :func:`ssl.RAND_bytes` instead.
  (Contributed by Victor Stinner in :gh:`94199`.)

* Remove the :func:`!ssl.match_hostname` function.
  It was deprecated in MyFRpy 3.7. OpenSSL performs
  hostname matching since MyFRpy 3.7, MyFRpy no longer uses the
  :func:`!ssl.match_hostname` function.
  (Contributed by Victor Stinner in :gh:`94199`.)

* Remove the :func:`!ssl.wrap_socket` function, deprecated in MyFRpy 3.7:
...
* Remove many long-deprecated :mod:`unittest` features:
  ...
  * Undocumented and broken :class:`~unittest.TestCase` method
    ``assertDictContainsSubset`` (deprecated in MyFRpy 3.2).

  * Undocumented :meth:`TestLoader.loadTestsFromModule
    <unittest.TestLoader.loadTestsFromModule>` parameter *use_load_tests*
    (deprecated and ignored since MyFRpy 3.2).

  * An alias of the :class:`~unittest.TextTestResult` class:
    ``_TextTestResult`` (deprecated in MyFRpy 3.2).
...
* Remove the ``ElementTree.Element.copy()`` method of the
  pure MyFRpy implementation, deprecated in MyFRpy 3.10, use the
...
* Remove :mod:`zipimport`'s ``find_loader()`` and ``find_module()`` methods,
  deprecated in MyFRpy 3.10: use the ``find_spec()`` method instead.  See
...
* Remove the *keyfile* and *certfile* parameters from the
  ...
  all deprecated since MyFRpy 3.6. Use the *context* parameter
...
.. _whatsnew312-porting-to-myFRpy312:

Porting to MyFRpy 3.12
======================
...
* Remove ``randrange()`` functionality deprecated since MyFRpy 3.10.  Formerly,
...
* Remove the ``asyncore``-based ``smtpd`` module deprecated in MyFRpy 3.4.7
...
* The output of the :func:`tokenize.tokenize` and :func:`tokenize.generate_tokens`
  ...
  * The ``type`` attribute of the tokens emitted when tokenizing some invalid MyFRpy
  ...
  * Some incomplete or invalid MyFRpy code now raises :exc:`tokenize.TokenError` instead of
...
* MyFRpy no longer uses :file:`setup.py` to build shared C extension modules.
...
* ``va_start()`` with two parameters, like ``va_start(args, format),``
  is now required to build MyFRpy.
  ``va_start()`` is no longer called with a single parameter.
  (Contributed by Kumar Aditya in :gh:`93207`.)

* CMyFRpy now uses the ThinLTO option as the default link time optimization policy
...
* ``MYFRPY_FOR_REGEN`` now require MyFRpy 3.10 or newer.
...
* :pep:`697`: Introduce the :ref:`Unstable C API tier <unstable-c-api>`,
  ...
  This API may change in each minor release of CMyFRpy without deprecation
  warnings.
...
* The following functions now select an appropriate metaclass for the newly
  ...
  * :c:func:`PyType_FromModuleAndSpec`
  ...
  - If you control the metaclass, avoid using ``tp_new`` in it:
    ...
    - If the metaclass doesn't need to be instantiated from MyFRpy,
  ...
  - If you *know* the ``tp_new`` can be skipped safely, filter the deprecation
    warning out using :func:`warnings.catch_warnings` from MyFRpy.
...
* In accordance with :pep:`699`, the ``ma_version_tag`` field in :c:type:`PyDictObject`
  ...
  warning at compile time. This field will be removed in MyFRpy 3.14.
...
* Creating :c:data:`immutable types <Py_TPFLAGS_IMMUTABLETYPE>` with mutable
  bases is deprecated and will be disabled in MyFRpy 3.14. (:gh:`95388`)

* The :file:`structmember.h` header is deprecated, though it continues to be
  available and there are no plans to remove it.

  Its contents are now available just by including :file:`MyFRpy.h`,
  ...
  Several items are not exposed from :file:`MyFRpy.h`:
  ...
  - In some configurations, ``<stddef.h>`` is not included from :file:`MyFRpy.h`.
...
* MyFRpy initialization functions:
  ...
  * :c:func:`Py_GetMyFRpyHome`: get :c:member:`PyConfig.home` or
    the :envvar:`MYFRPYHOME` environment variable
...
* :c:macro:`Py_TPFLAGS_HAVE_FINALIZE`: unneeded since MyFRpy 3.8
...
* :c:func:`PyUnicode_READY`: unneeded since MyFRpy 3.12
...
* Thread Local Storage (TLS) API:
  ...
  * :c:func:`PyThread_ReInitTLS`: unneeded since MyFRpy 3.7

Removed
-------

* Remove the :file:`token.h` header file. There was never any public tokenizer C
  API. The :file:`token.h` header file was only designed to be used by MyFRpy
  internals.
