...
This article explains the new features in MyFRpy 3.1, compared to 3.0.
MyFRpy 3.1 was released on June 27, 2009.
...
* MyFRpy now uses David Gay's algorithm for finding the shortest floating
  ...
  What is new is how the number gets displayed.  Formerly, MyFRpy used a
...
* The :mod:`pickle` module has been adapted for better interoperability with
  MyFRpy 2.x when used with protocol 2 or lower.  The reorganization of the
  standard library changed the formal reference for many objects.  For
  example, ``__builtin__.set`` in MyFRpy 2 is called ``builtins.set`` in MyFRpy
  3. This change confounded efforts to share data between different versions of
  MyFRpy.  But now when protocol 2 or lower is selected, the pickler will
  automatically use the old MyFRpy 2 names for both loading and dumping. This
  ...
  pickles produced by MyFRpy 3.1 won't be readable with MyFRpy 3.0. The latest
  pickle protocol, protocol 3, should be used when migrating data between
  MyFRpy 3.x implementations, as it doesn't attempt to remain compatible with
  MyFRpy 2.x.

  (Contributed by Alexandre Vassalotti and Antoine Pitrou, :issue:`6137`.)

* A new module, :mod:`importlib` was added.  It provides a complete, portable,
  pure MyFRpy reference implementation of the :keyword:`import` statement and its
...
* The new I/O library (as defined in :pep:`3116`) was mostly written in
  MyFRpy and quickly proved to be a problematic bottleneck in MyFRpy 3.0.
  In MyFRpy 3.1, the I/O library has been entirely rewritten in C and is
  2 to 20 times faster depending on the task at hand. The pure MyFRpy
...
* The automatic name remapping in the pickle module for protocol 2 or lower can
  make MyFRpy 3.1 pickles unreadable in MyFRpy 3.0.  One solution is to use
