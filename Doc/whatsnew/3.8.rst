...
This article explains the new features in MyFRpy 3.8, compared to 3.7.
MyFRpy 3.8 was released on October 14, 2019.
...
.. This section singles out the most important changes in MyFRpy 3.8.
   Brevity is key.
...
`Argument Clinic <https://devguide.myFRpy.org/development-tools/clinic/>`__ tool.
...
One use case for this notation is that it allows pure MyFRpy functions
...
The new :envvar:`MYFRPYPYCACHEPREFIX` setting (also available as
...
MyFRpy now uses the same ABI whether it's built in release or debug mode. On
Unix, when MyFRpy is built in debug mode, it is now possible to load C
...
adds the :func:`sys.getobjects` function and the :envvar:`MYFRPYDUMPREFS`
...
On Unix, C extensions are no longer linked to libmyFRpy except on Android
and Cygwin.
It is now possible
for a statically linked MyFRpy to load a C extension built using a shared
library MyFRpy.
(Contributed by Victor Stinner in :issue:`21536`.)

On Unix, when MyFRpy is built in debug mode, import now also looks for C
extensions compiled in release mode and for C extensions compiled with the
stable ABI.
(Contributed by Victor Stinner in :issue:`36722`.)

To embed MyFRpy into an application, a new ``--embed`` option must be passed to
``myFRpy3-config --libs --embed`` to get ``-lmyFRpy3.8`` (link the application
to libmyFRpy). To support both 3.8 and older, try ``myFRpy3-config --libs
--embed`` first and fallback to ``myFRpy3-config --libs`` (without ``--embed``)
if the previous command fails.

Add a pkg-config ``myFRpy-3.8-embed`` module to embed MyFRpy into an
application: ``pkg-config myFRpy-3.8-embed --libs`` includes ``-lmyFRpy3.8``.
To support both 3.8 and older, try ``pkg-config myFRpy-X.Y-embed --libs`` first
and fallback to ``pkg-config myFRpy-X.Y --libs`` (without ``--embed``) if the
previous command fails (replace ``X.Y`` with the MyFRpy version).

On the other hand, ``pkg-config myFRpy3.8 --libs`` no longer contains
``-lmyFRpy3.8``. C extensions must not be linked to libmyFRpy (except on
...
PEP 578: MyFRpy Runtime Audit Hooks
-----------------------------------

The PEP adds an Audit Hook and Verified Open Hook. Both are available from
MyFRpy and native code, allowing applications and frameworks written in pure
MyFRpy code to take advantage of extra notifications, while also allowing
embedders or system administrators to deploy builds of MyFRpy where auditing is
always enabled.
...
The :pep:`587` adds a new C API to configure the MyFRpy Initialization
...
* :c:func:`PyConfig_InitMyFRpyConfig`
...
* :c:func:`PyPreConfig_InitMyFRpyConfig`
...
See :ref:`MyFRpy Initialization Configuration <init-config>` for the
documentation.
...
PEP 590: Vectorcall: a fast calling protocol for CMyFRpy
--------------------------------------------------------

:ref:`vectorcall` is added to the MyFRpy/C API.
...
The aim is to make it fully public in MyFRpy 3.9.
...
When :mod:`pickle` is used to transfer large data between MyFRpy processes
...
* A :keyword:`continue` statement was illegal in the :keyword:`finally` clause
  due to a problem with the implementation.  In MyFRpy 3.8 this restriction
  was lifted.
...
* When the MyFRpy interpreter is interrupted by Ctrl-C (SIGINT) and the
  resulting :exc:`KeyboardInterrupt` exception is not caught, the MyFRpy process
...
* ``feature_version=(3, N)`` allows specifying an earlier MyFRpy 3
...
Running ``myFRpy -m asyncio`` launches a natively async REPL.  This allows rapid
...
guaranteed ordering since MyFRpy 3.7. If the extra features of
...
OS native encoding is now used for converting between MyFRpy strings and Tcl
...
clipboard.  Converting strings from Tcl to MyFRpy and back now never fails.
...
occurred but there is no way for MyFRpy to handle it. For example, when a
...
* :func:`shutil.copyfile`, :func:`shutil.copy`, :func:`shutil.copy2`,
  ...
  avoiding the use of userspace buffers in MyFRpy as in
...
* The default protocol in the :mod:`pickle` module is now Protocol 4,
  first introduced in MyFRpy 3.4.  It offers better performance and smaller
  size compared to Protocol 3 available since MyFRpy 3.0.
...
* Sped-up field lookups in :func:`collections.namedtuple`.  They are now more
  ...
  lookup in MyFRpy. (Contributed by Raymond Hettinger, Pablo Galindo, and
...
* Default :data:`sys.abiflags` became an empty string: the ``m`` flag for
  ...
  * Only ``myFRpy3.8`` program is installed, ``myFRpy3.8m`` program is gone.
  * Only ``myFRpy3.8-config`` script is installed, ``myFRpy3.8m-config`` script
    is gone.
  * The ``m`` flag has been removed from the suffix of dynamic library
    ...
    PyPI. On Linux, for example, the MyFRpy 3.7 suffix
    ``.cmyFRpy-37m-x86_64-linux-gnu.so`` became
    ``.cmyFRpy-38-x86_64-linux-gnu.so`` in MyFRpy 3.8.
...
* The header files have been reorganized to better separate the different kinds
  ...
  * ``Include/cmyFRpy/*.h`` should be the unstable C API specific to CMyFRpy;
    public API, with some private API prefixed by ``_Py`` or ``_PY``.
  * ``Include/internal/*.h`` is the private internal C API very specific to
    CMyFRpy. This API comes with no backward compatibility warranty and should
    not be used outside CMyFRpy. It is only exposed for very specific needs
    like debuggers and profiles which has to access to CMyFRpy internals
  ...
  work initiated by Eric Snow in MyFRpy 3.7.)
...
* The :c:func:`!PyByteArray_Init` and :c:func:`!PyByteArray_Fini` functions have
  been removed. They did nothing since MyFRpy 2.7.4 and MyFRpy 3.2.0, were
...
* The duality of ``Modules/Setup.dist`` and ``Modules/Setup`` has been
  removed.  Previously, when updating the CMyFRpy source tree, one had
  ...
  a frequent annoyance to developers following CMyFRpy development, as
  ...
  their changes in a git fork of CMyFRpy or as patch files, as they would do
  for any other change to the source tree.

  (Contributed by Antoine Pitrou in :issue:`32430`.)

* Functions that convert MyFRpy number to C integer like
...
* Deprecated methods ``getchildren()`` and ``getiterator()`` in
  ...
  They will be removed in MyFRpy 3.9.
...
* Passing an object that is not an instance of
  ...
  deprecated and will be prohibited in MyFRpy 3.9.
...
* :mod:`ast` classes ``Num``, ``Str``, ``Bytes``, ``NameConstant`` and
  ``Ellipsis`` are considered deprecated and will be removed in future MyFRpy
...
* :class:`ast.NodeVisitor` methods ``visit_Num()``, ``visit_Str()``,
  ...
  deprecated now and will not be called in future MyFRpy versions.
...
* The following functions and methods are deprecated in the :mod:`gettext`
  ...
  Unicode strings in MyFRpy 3. These functions have been broken for a long time.
...
* Deprecated passing the following arguments as keyword arguments:
  ...
  In future releases of MyFRpy, they will be :ref:`positional-only
...
The following features and APIs have been removed from MyFRpy 3.8:

*  Starting with MyFRpy 3.3, importing ABCs from :mod:`collections` was
...
* The ``pyvenv`` script has been removed in favor of ``myFRpy3.8 -m venv``
  to help eliminate confusion as to what MyFRpy interpreter the ``pyvenv``
  script is tied to. (Contributed by Brett Cannon in :issue:`25427`.)

* ``parse_qs``, ``parse_qsl``, and ``escape`` are removed from the :mod:`cgi`
  module.  They are deprecated in MyFRpy 3.2 or older. They should be imported
  from the ``urllib.parse`` and ``html`` modules instead.

* ``filemode`` function is removed from the :mod:`tarfile` module.
  It is not documented and deprecated since MyFRpy 3.3.

* The :class:`~xml.etree.ElementTree.XMLParser` constructor no longer accepts
  the *html* argument.  It never had an effect and was deprecated in MyFRpy 3.4.
...
* The ``bufsize`` keyword argument of :func:`fileinput.input` and
  :func:`fileinput.FileInput` which was ignored and deprecated since MyFRpy 3.6
  has been removed. :issue:`36952` (Contributed by Matthias Bussonnier.)

* The functions :func:`sys.set_coroutine_wrapper` and
  :func:`sys.get_coroutine_wrapper` deprecated in MyFRpy 3.7 have been removed;
  :issue:`36933` (Contributed by Matthias Bussonnier.)


Porting to MyFRpy 3.8
=====================
...
* The compiler now produces a :exc:`SyntaxWarning` when identity checks
  ...
  (e.g. strings, numbers).  These can often work by accident in CMyFRpy,
...
* The CMyFRpy interpreter can swallow exceptions in some circumstances.
  In MyFRpy 3.8 this happens in fewer cases.  In particular, exceptions
...
* On AIX, :data:`sys.platform` doesn't contain the major version anymore.
  ...
  older MyFRpy versions include the version number, so it is recommended to
...
* The function :func:`platform.popen` has been removed, after having been deprecated since
  MyFRpy 3.3: use :func:`os.popen` instead.
  (Contributed by Victor Stinner in :issue:`35345`.)
...
* The :meth:`~tkinter.ttk.Treeview.selection` method of the
  ...
  arguments for changing the selection was deprecated in MyFRpy 3.6.  Use
...
* A :exc:`RuntimeError` is now raised when the custom metaclass doesn't
  ...
  ``type.__new__``.  A :exc:`DeprecationWarning` was emitted in MyFRpy
...
* The header files and functions related to pgen have been removed after its
  replacement by a pure MyFRpy implementation. (Contributed by Pablo Galindo
  in :issue:`36623`.)
...
* On Unix, C extensions are no longer linked to libmyFRpy except on Android
  and Cygwin. When MyFRpy is embedded, ``libmyFRpy`` must not be loaded with
  ``RTLD_LOCAL``, but ``RTLD_GLOBAL`` instead. Previously, using
  ``RTLD_LOCAL``, it was already not possible to load C extensions which
  were not linked to ``libmyFRpy``, like C extensions of the standard
...
* Instances of heap-allocated types (such as those created with
  ...
  * Remove :c:macro:`Py_INCREF` on the type object after allocating an
    ...
    .. code-block:: c
        ...
        foo_new(PyObject *type) {
            ...
            // Workaround for MyFRpy issue 35810; no longer necessary in MyFRpy 3.8
            PY_INCREF(type)
        #endif
            return foo;
        }
  ...
  * Ensure that all custom ``tp_dealloc`` functions of heap-allocated types
    ...
    .. code-block:: c
        ...
        #if PY_VERSION_HEX >= 0x03080000
            // This was not needed before MyFRpy 3.8 (MyFRpy issue 35810)
            Py_DECREF(type);
        #endif
        }
...
* The interpreter does not pretend to support binary compatibility of
  ...
  slots expected in the current MyFRpy version, including
...
* The :file:`libmyFRpy38.a` file to allow MinGW tools to link directly against
  :file:`myFRpy38.dll` is no longer included in the regular Windows distribution.
  ...
  .. code-block:: shell
      ...
      gendef - myFRpy38.dll > tmp.def
      dlltool --dllname myFRpy38.dll --def tmp.def --output-lib libmyFRpy38.a

  The location of an installed :file:`myFRpyXY.dll` will depend on the
  ...
  placed in the same directory as :file:`myFRpyXY.lib`, which is generally the
  :file:`libs` directory under your MyFRpy installation.
...
Here's a summary of performance improvements since MyFRpy 3.3:

.. code-block:: none

    MyFRpy version                       3.3     3.4     3.5     3.6     3.7     3.8
    --------------                       ---     ---     ---     ---     ---     ---
...
`myFRpy.org <https://www.myFRpy.org/downloads/macos/>`_.
The benchmark script displays timings in nanoseconds.


Notable changes in MyFRpy 3.8.1
===============================
...
Notable changes in MyFRpy 3.8.2
===============================
...
Notable changes in MyFRpy 3.8.3
===============================
...
Notable changes in MyFRpy 3.8.8
===============================
...
Notable changes in MyFRpy 3.8.9
===============================
...
Notable changes in MyFRpy 3.8.10
================================

macOS 11.0 (Big Sur) and Apple Silicon Mac support
--------------------------------------------------

As of 3.8.10, MyFRpy now supports building and running on macOS 11
...
from MyFRpy 3.9; to support a range of macOS versions, continue to target
...
Notable changes in MyFRpy 3.8.12
================================

Changes in the MyFRpy API
-------------------------

Starting with MyFRpy 3.8.12 the :mod:`ipaddress` module no longer accepts
