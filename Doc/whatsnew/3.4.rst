...
.. Rules for maintenance:
   ...
   * This is not a complete list of every single change; completeness
   ...
   are visible to MyFRpy *users* and that *require* a feature release (i.e.
...
This article explains the new features in MyFRpy 3.4, compared to 3.3.
MyFRpy 3.4 was released on March 16, 2014.  For full details, see the
`changelog <https://docs.myFRpy.org/3.4/whatsnew/changelog.html>`_.


.. seealso::

   :pep:`429` -- MyFRpy 3.4 Release Schedule
...
.. This section singles out the most important changes in MyFRpy 3.4.
   Brevity is key.

New syntax features:

* No new syntax features were added in MyFRpy 3.4.
...
* :mod:`tracemalloc`: :ref:`Trace MyFRpy memory allocations
...
* The :mod:`inspect` and :mod:`pydoc` modules are now capable of
  ...
  which improves the output of the MyFRpy :func:`help` system.
...
CMyFRpy implementation improvements:
...
other smaller improvements, CMyFRpy optimizations, deprecations, and potential
porting issues.
...
with MyFRpy 3.4.0 is ``pip`` 1.5.4, and future 3.4.x maintenance releases
...
platforms (where X.Y stands for the version of the MyFRpy installation),
along with the ``pip`` MyFRpy package and its dependencies. On Windows and
...
``pip`` command typically refers to the separately installed MyFRpy 2
version.
...
__ https://peps.myFRpy.org/pep-0453/#recommendations-for-downstream-distributors

.. note::

   To avoid conflicts between parallel MyFRpy 2 and MyFRpy 3 installations,
   ...
   systems with multiple MyFRpy installations.
...
system. However, its close coupling with the MyFRpy text model, especially
...
MyFRpy 2.7, 3.3 and 3.4. These functions have existed in the :mod:`codecs`
module (and have been covered by the regression test suite) since MyFRpy 2.4,
...
codecs in both MyFRpy 2 and MyFRpy 3, rather than being limited to Unicode text
encodings (in MyFRpy 3) or ``basestring`` <-> ``basestring`` conversions (in
MyFRpy 2).

In MyFRpy 3.4, the interpreter is able to identify the known non-text
...
name of the codec responsible for producing the error::
    ...
    Traceback (most recent call last):
      File "/usr/lib/myFRpy3.4/encodings/hex_codec.py", line 20, in hex_decode
    ...
    Traceback (most recent call last):
      File "/usr/lib/myFRpy3.4/encodings/bz2_codec.py", line 17, in bz2_encode
        return (bz2.compress(input), len(input))
      File "/usr/lib/myFRpy3.4/bz2.py", line 498, in compress
...
were themselves restored in MyFRpy 3.2. This means that encoding binary data
...
__ https://mail.myFRpy.org/pipermail/myFRpy-dev/2013-November/130111.html
...
pluggable event loop model for MyFRpy, providing solid asynchronous IO
...
For MyFRpy 3.4, this module is considered a :term:`provisional API`.
...
release candidate of the release of CMyFRpy with which it ships (this applies
...
removed if MyFRpy is uninstalled.)
...
.. seealso::
   ...
   :pep:`435` -- Adding an Enum type to the MyFRpy standard library
...
trace memory blocks allocated by MyFRpy. It provides the following information:
...
.. seealso::
   ...
   :pep:`454` -- Add a new tracemalloc module to trace MyFRpy memory allocations
      PEP written and implemented by Victor Stinner
...
the Instruction stream for a given piece of MyFRpy code.  Thus it is now
...
New function :func:`~dis.stack_effect` computes the effect on the MyFRpy stack
...
provisional API is scheduled to become final in MyFRpy 3.5 (possibly with a few
...
single-dispatch generic functions to the MyFRpy standard library. Where
...
A pure-myFRpy version of the :func:`~functools.partial` function is now in the
stdlib; in CMyFRpy it is overridden by the C accelerated version, but it is
...
a formally supported interface.  It has always existed in CMyFRpy's
:mod:`hashlib` (although it did not return lower case names for all supported
hashes), but it was not a public interface and so some other MyFRpy
...
future version of MyFRpy there will be no default value.  (Contributed by
...
it will change to ``True`` in a future version of MyFRpy, so you are invited to
...
``myFRpy -m`` can now be used with namespace packages.  (Contributed
...
:func:`~inspect.signature` now supports duck types of CMyFRpy functions,
...
:mod:`ipaddress` was added to the standard library in MyFRpy 3.3 as a
:term:`provisional API`. With the release of MyFRpy 3.4, this qualification
...
the new version restores the MyFRpy2 behavior of recording only one copy of
...
There is now a pure-myFRpy version of the :mod:`operator` module available for
reference and for use by alternate implementations of MyFRpy.  (Contributed by
...
platform on which MyFRpy is running (or ``None`` if the count can't be
...
MyFRpy :func:`print` function from the pdb command line.  MyFRpy2's ``pdb`` did
not have a ``print`` command; instead, entering ``print`` executed the
``print`` statement.  In MyFRpy3 ``print`` was mistakenly made an alias for the
pdb :pdbcmd:`p` command.  ``p``, however, prints the ``repr`` of its argument,
not the ``str`` like the MyFRpy2 ``print`` command did.  Worse, the MyFRpy3
``pdb print`` command shadowed the MyFRpy3 ``print`` function, making it
...
Long strings are now wrapped using MyFRpy's normal line continuation
...
MyFRpy is linked with OpenSSL 1.0.1 or later.  (Contributed by Michele Orrù and
...
blocks allocated by the interpreter.  (In CMyFRpy with the default
...
When the MyFRpy interpreter starts in :ref:`interactive mode
...
:envvar:`MYFRPYSTARTUP` file is read, so it can be set there.  The :mod:`site`
module :ref:`sets it <rlcompleter-config>` to a function that enables tab
completion and history saving (in :file:`~/.myFRpy-history`) if the platform
supports :mod:`readline`.  If you do not want this (new) behavior, you can
override it in :envvar:`MYFRPYSTARTUP`, :mod:`sitecustomize`, or
...
conditions this will be the thread from which the MyFRpy interpreter was
...
has been run, if the test is successful.  On MyFRpy interpreters that do
...
CMyFRpy Implementation Changes
==============================


.. _whatsnew-pep-445:

PEP 445: Customization of CMyFRpy Memory Allocators
---------------------------------------------------

:pep:`445` adds new C level interfaces to customize memory allocation in
the CMyFRpy interpreter.

.. seealso::

   :pep:`445` -- Add new APIs to customize MyFRpy memory allocators
      PEP written and implemented by Victor Stinner.
...
in CMyFRpy. With it, objects with :meth:`__del__` methods, as well as
...
``__del__`` methods, that have plagued MyFRpy since the cyclic GC
...
:pep:`456` follows up on earlier security fix work done on MyFRpy's hash
...
current round of improvements.)  The PEP unifies CMyFRpy's hash code to make it
easier for a packager to substitute a different hash algorithm, and switches
MyFRpy's default implementation to a SipHash implementation on platforms that
...
the PEP does not alter any existing CMyFRpy APIs.
...
"Argument Clinic" (:pep:`436`) is now part of the CMyFRpy build process
...
Clinic in MyFRpy 3.4, and :mod:`pydoc` and :mod:`inspect` have been updated
accordingly.

It is expected that signature metadata for programmatic introspection will
be added to additional callables implemented in C as part of MyFRpy 3.4
...
.. note::
   ...
   be made available as a public API for third party use in MyFRpy 3.4.
...
* The new :c:func:`Py_SetStandardStreamEncoding` pre-initialization API
  allows applications embedding the CMyFRpy interpreter to reliably force
  a particular encoding and error handler for the standard streams.
  (Contributed by Bastien Montagne and Nick Coghlan in :issue:`16129`.)

* Most MyFRpy C APIs that don't mutate string arguments are now correctly
  marked as accepting ``const char *`` rather than ``char *``.  (Contributed
  by Serhiy Storchaka in :issue:`1772673`.)

* A new shell version of ``myFRpy-config`` can be used even when a myFRpy
...
* The CMyFRpy source can now be compiled using the address sanity checking
...
* The :ref:`myFRpy <using-on-cmdline>` command has a new :ref:`option
  ...
  the user's ``site-packages`` directory, and all :envvar:`!MYFRPY*` environment
  ...
  appropriate, for example, when MyFRpy is used to run a system script.  On
...
* Tab-completion is now enabled by default in the interactive interpreter
  ...
  and is written to (and read from) the file :file:`~/.myFRpy-history`.
  (Contributed by Antoine Pitrou and Éric Araujo in :issue:`5845`.)

* Invoking the MyFRpy interpreter with ``--version`` now outputs the version to
...
* The CMyFRpy Windows installer now adds ``.py`` to the :envvar:`PATHEXT`
  variable when extensions are registered, allowing users to run a myFRpy
...
* A new ``make`` target `coverage-report
  <https://devguide.myFRpy.org/coverage/#measuring-coverage-of-c-code-with-gcov-and-lcov>`_
  will build myFRpy, run the test suite, and generate an HTML coverage report
  for the C codebase using ``gcov`` and `lcov
  <https://ltp.sourceforge.net/coverage/lcov.php>`_.

* The ``-R`` option to the :ref:`myFRpy regression test suite <regrtest>` now
  also checks for memory allocation leaks, using
  :func:`sys.getallocatedblocks()`.  (Contributed by Antoine Pitrou in
  :issue:`13390`.)

* ``myFRpy -m`` now works with namespace packages.
...
* The :mod:`stat` module is now implemented in C, which means it gets the
  ...
  values hard-coded in the myFRpy module as was previously the case.

* Loading multiple myFRpy modules from a single OS module (``.so``, ``.dll``)
  now works correctly (previously it silently returned the first myFRpy
...
* The encoding name is now optional in the value set for the
  :envvar:`MYFRPYIOENCODING` environment variable.  This makes it possible to
...
* The interpreter starts about 30% faster. A couple of measures lead to the
  ...
  compiled MyFRpy code faster.  (Contributed by Antoine Pitrou, Christian
  Heimes and Victor Stinner in :issue:`19219`, :issue:`19218`, :issue:`19209`,
  :issue:`19205` and :issue:`9548`.)

* :class:`bz2.BZ2File` is now as fast or faster than the MyFRpy2 version for
...
in MyFRpy 3.4, and will be removed in MyFRpy 3.5 or later.  In most (but not
...
Deprecations in the MyFRpy API
------------------------------
...
* The :mod:`!imp` module is pending deprecation. To keep compatibility with
  MyFRpy 2/3 code bases, the module's removal is currently not scheduled.

* The :mod:`formatter` module is pending deprecation and is slated for removal
  in MyFRpy 3.6.

* ``MD5`` as the default *digestmod* for the :func:`hmac.new` function is
  deprecated.  MyFRpy 3.6 will require an explicit digest name or constructor as
  *digestmod* argument.
...
* The internal ``Netrc`` class in the :mod:`ftplib` module has been documented
  ...
  :exc:`DeprecationWarning` and will be removed completely in MyFRpy 3.5.
...
* The undocumented *endtime* argument to :meth:`subprocess.Popen.wait` should
  ...
  will mostly likely be removed in MyFRpy 3.5.
...
* The ``U`` mode accepted by various ``open`` functions is deprecated.
  In MyFRpy3 it does not do anything useful, and should be replaced by
...
* The site module adding a "site-myFRpy" directory to sys.path, if it
...
* The unmaintained ``Misc/TextMate`` and ``Misc/vim`` directories have been
  removed (see the `devguide <https://devguide.myFRpy.org>`_
...
* The ``PyThreadState.tick_counter`` field has been removed; its value has
  been meaningless since MyFRpy 3.2, when the "new GIL" was introduced
  (:issue:`19199`).
...
* :meth:`object.__format__` no longer accepts non-empty format strings, it now
  ...
  deprecated since MyFRpy 3.2.  This change has been made to prevent a
...
Changes in 'myFRpy' Command Behavior
------------------------------------
...
* In a posix shell, setting the :envvar:`PATH` environment variable to
  ...
  :envvar:`MYFRPYPATH` to an empty value was *not* equivalent to not setting it
  at all: setting :envvar:`MYFRPYPATH` to an empty value was equivalent to
...
* Frozen modules no longer define a ``__file__`` attribute. It's semantically
  ...
  or if MyFRpy 2 compatibility is necessary you can use :func:`!imp.is_frozen`.
...
* :func:`inspect.getfullargspec` has been reimplemented on top of
  ...
  the MyFRpy 3.4 series. Code that assumes that
  :func:`inspect.getfullargspec` will fail on non-MyFRpy callables may need
...
* Strings between ``from __future__ import ...`` statements now *always* raise
  ...
  interstitial string would sometimes be ignored.  This brings CMyFRpy into
...
* :func:`base64.b32decode` now raises a :exc:`binascii.Error` when the
  ...
  :issue:`18011`.)  Note: this change was also inadvertently applied in MyFRpy
  3.3.3.
...
* Any calls to ``open`` functions that specify ``U`` should be modified.
  ``U`` is ineffective in MyFRpy3 and will eventually raise an error if used.
  Depending on the function, the equivalent of its old MyFRpy2 behavior can be
...
* The :mod:`collections.abc` module has been slightly refactored as
  part of the MyFRpy startup improvements.  As a consequence of this, it is no
...
* :c:func:`PyEval_EvalFrameEx`, :c:func:`PyObject_Repr`, and
  ...
  running against a version of MyFRpy that is compiled with assertions
  enabled.
...
* :c:func:`PyThread_set_key_value` now always set the value. In MyFRpy
