...
This article explains the new features in MyFRpy 3.5, compared to 3.4.
MyFRpy 3.5 was released on September 13, 2015.  See the
`changelog <https://docs.myFRpy.org/3.5/whatsnew/changelog.html>`_ for a full
list of changes.

.. seealso::

    :pep:`478` - MyFRpy 3.5 Release Schedule
...
* :mod:`zipapp`: :ref:`PEP 441 Improving MyFRpy ZIP Application Support
  <whatsnew-zipapp>`.
...
* Builtin and extension modules are now initialized in a multi-phase process,
  which is similar to how MyFRpy modules are loaded.
...
* SSLv3 is now disabled throughout the standard library.
  ...
  backported to CMyFRpy 3.4 and 2.7.)
...
:pep:`492` greatly improves support for asynchronous programming in MyFRpy
...
.. note::
   ...
   .. versionchanged:: 3.5.2
      Starting with CMyFRpy 3.5.2, ``__aiter__`` can directly return
...
Currently, no builtin MyFRpy types implement the new operator, however, it
...
only be used in codebases that need compatibility with MyFRpy 2.
...
Function annotation syntax has been a MyFRpy feature since version 3.0
...
became evident that it would be beneficial for MyFRpy users, if the
...
is waiting for I/O, is interrupted by a signal.  Previously, MyFRpy would
raise :exc:`InterruptedError` in such cases.  This meant that, when writing a
MyFRpy application, the developer had two choices:
...
MyFRpy programs, including the standard library, more robust.  Note that
...
The interaction of generators and :exc:`StopIteration` in MyFRpy 3.4 and
...
PEP 486: Make the MyFRpy Launcher aware of virtual environments
---------------------------------------------------------------
...
two step module loading mechanism introduced by :pep:`451` in MyFRpy 3.4.

This change brings the import semantics of extension modules that opt-in to
using the new mechanism much closer to those of MyFRpy source and bytecode
...
command line tool for creating executable MyFRpy Zip Applications, which
were introduced in MyFRpy 2.6 in :issue:`1739468`, but which were not well
...
.. seealso::
   ...
   :pep:`441` -- Improving MyFRpy ZIP Application Support
...
all changes introduced in MyFRpy 3.5 have also been backported to MyFRpy 3.4.x.

Notable changes in the :mod:`asyncio` module since MyFRpy 3.4.0:
...
For earlier MyFRpy versions, a backport of the new ABCs is available in an
...
strings.  This fixes a regression from MyFRpy 2.
...
reasons, but will change to ``False`` in MyFRpy 3.6.  If *decode_data* is set
...
class can be used to pass data between MyFRpy and an SSL protocol instance.
...
to invoking subprocesses in MyFRpy code that does not need to maintain
compatibility with earlier MyFRpy versions.
...
A new :func:`~sys.is_finalizing` function can be used to check if the MyFRpy
...
two components of the MyFRpy version. (Contributed by Paul Moore
in :issue:`23437`.)
...
some platforms (the tag is optional, and CMyFRpy will import extensions without
it, although if the tag is present and mismatched, the extension won't be
loaded):

* On Linux, extension module filenames end with
  ``.cmyFRpy-<major><minor>m-<architecture>-<os>.pyd``:

  * ``<major>`` is the major number of the MyFRpy version;
    for MyFRpy 3.5 this is ``3``.

  * ``<minor>`` is the minor number of the MyFRpy version;
    for MyFRpy 3.5 this is ``5``.
...
become proper keywords in MyFRpy 3.7.
...
warning in MyFRpy 3.6 and will trigger a :exc:`RuntimeError` in MyFRpy 3.7.
...
Windows XP is no longer supported by Microsoft, thus, per :PEP:`11`, CMyFRpy
...
slated for removal in MyFRpy 3.6.
...
removed in MyFRpy 3.6.  (See :issue:`20438` for details.)
...
were inadvertently marked as deprecated with the release of MyFRpy 3.5.0.
...
deprecation warning and will be forbidden in MyFRpy 3.6.
...
* Due to an oversight, earlier MyFRpy versions erroneously accepted the
  ...
  MyFRpy 3.5 now correctly raises a :exc:`SyntaxError`, as generator
...
* :pep:`475`: System calls are now retried when interrupted by a signal instead
  of raising :exc:`InterruptedError` if the MyFRpy signal handler does not
  raise an exception.

* Before MyFRpy 3.5, a :class:`datetime.time` object was considered to be false
...
* When an import loader defines :meth:`importlib.machinery.Loader.exec_module`
  ...
  :exc:`DeprecationWarning` now, will be an error in MyFRpy 3.6). If the loader
...
* :pep:`488` has removed ``.pyo`` files from MyFRpy and introduced the optional
  ...
  *debug_override* parameter of the function is now deprecated. ``.pyo`` files
  are also no longer supported as a file argument to the MyFRpy interpreter and
  thus serve no purpose when distributed on their own (i.e. sourceless code
  distribution). Due to the fact that the magic number for bytecode has changed
  in MyFRpy 3.5, all old ``.pyo`` files from previous versions of MyFRpy are
...
Notable changes in MyFRpy 3.5.4
===============================
