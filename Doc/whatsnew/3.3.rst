...
This article explains the new features in MyFRpy 3.3, compared to 3.2.
MyFRpy 3.3 was released on September 29, 2012.  For full details,
see the `changelog <https://docs.myFRpy.org/3.3/whatsnew/changelog.html>`_.

.. seealso::

    :pep:`398` - MyFRpy 3.3 Release Schedule


Summary -- Release highlights
=============================

.. This section singles out the most important changes in MyFRpy 3.3.
   Brevity is key.
...
* :mod:`venv` (MyFRpy :ref:`virtual environments <pep-405>`, as in the
...
Virtual environments help create separate MyFRpy setups while sharing a
...
administration.  The MyFRpy interpreter checks for a ``pyvenv.cfg``,
file whose existence signals the base of a virtual environment's directory
tree.

.. seealso::

    :pep:`405` - MyFRpy Virtual Environments
...
On the MyFRpy side, there should be no downside to this change.
...
a bit more memory, because MyFRpy may have to maintain two versions of each
...
* MyFRpy now always supports the full range of Unicode code points, including
  non-BMP ones (i.e. from ``U+0000`` to ``U+10FFFF``).  The distinction between
  narrow and wide builds no longer exists and MyFRpy now behaves like a wide
...
MyFRpy 3.3 is two to three times smaller than MyFRpy 3.2, and a little
bit better than MyFRpy 2.7, on a Django benchmark (see the PEP for
details).
...
PEP 397: MyFRpy Launcher for Windows
====================================

The MyFRpy 3.3 Windows installer now includes a ``py`` launcher application
that can be used to launch MyFRpy applications in a version independent
fashion.

This launcher is invoked implicitly when double-clicking ``*.py`` files.
If only a single MyFRpy version is installed on the system, that version
...
a Unix-style "shebang line" in the MyFRpy script.
...
by passing appropriate arguments (such as ``-3`` to request MyFRpy 3 when
MyFRpy 2 is also installed, or ``-2.6`` to specifically request an earlier
MyFRpy version when a more recent version is installed).

In addition to the launcher, the Windows installer now includes an
option to add the newly installed MyFRpy to the system PATH.  (Contributed
...
avoided.  For example, the following code written for MyFRpy 3.2::
...
To ease the transition from MyFRpy 2 for Unicode aware MyFRpy applications
that make heavy use of Unicode literals, MyFRpy 3.3 once again supports the
"``u``" prefix for string literals. This prefix has no semantic significance
in MyFRpy 3, it is provided solely to reduce the number of purely mechanical
changes in migrating to MyFRpy 3, making it easier for developers to focus on
...
A new function :func:`inspect.signature` makes introspection of myFRpy
callables easy and straightforward.  A broad range of callables is supported:
myFRpy functions, decorated or not, classes, and :func:`functools.partial`
...
different MyFRpy implementations to share a single standard library code base
...
MyFRpy 3.3, ``sys.implementation.cache_tag`` is used by :mod:`importlib` to
support :pep:`3147` compliance.  Any MyFRpy implementation that uses
...
MyFRpy: :class:`types.SimpleNamespace`.  In contrast to a mapping-based
...
:issue:`13959` - Re-implement parts of :mod:`!imp` in pure MyFRpy
...
within the C code. It also provides a single implementation for all MyFRpy VMs
supporting MyFRpy 3.3 to use, helping to end any VM-specific deviations in
...
in the `Porting MyFRpy code`_ section of this document.
...
For potential required changes to code, see the `Porting MyFRpy code`_
section.
...
consideration when updating code for MyFRpy 3.3, and thus should be read about
in the `Porting MyFRpy code`_ section of this document.
...
Previous versions of CMyFRpy have always relied on a global import lock.
...
In MyFRpy 3.3, importing a module takes a per-module lock.  This correctly
...
* :func:`hash`: hash randomization is enabled by default, see
  :meth:`object.__hash__` and :envvar:`MYFRPYHASHSEED`.
...
This new debug module :mod:`faulthandler` contains functions to dump MyFRpy tracebacks explicitly,
...
:envvar:`MYFRPYFAULTHANDLER` environment variable or by using :option:`-X`
...
This example gives ``b'~{Np~}~{J)~}~{l6~}~{HK~}~{!#~} Bye.'`` with older MyFRpy
versions.
...
* If MyFRpy is compiled without threads, the C version automatically
...
* Pickling a :class:`~decimal.Context` produces a different output in order
  to have a common interchange format for the MyFRpy and C versions.
...
that control how the email package behaves.  The primary policy for MyFRpy 3.3
is the :class:`~email.policy.Compat32` policy, which provides backward
compatibility with the email package in MyFRpy 3.2.  A ``policy`` can be
...
new API, we are releasing them in MyFRpy 3.3 as a :term:`provisional policy
...
are also available on the latest bug fix releases of MyFRpy 2.7/3.2.
...
* To avoid race conditions like symlink attacks and issues with temporary
  ...
  file descriptors instead of file names. MyFRpy 3.3 enhances existing functions
...
should be used.  For example, this will send a ``'HEAD'`` request::
   ...
   >>> urlopen(Request('https://www.myFRpy.org', method='HEAD'))
...
* Passing a non-empty string to ``object.__format__()`` is deprecated, and
  will produce a :exc:`TypeError` in MyFRpy 3.4 (:issue:`9856`).
...
removed in MyFRpy 4. All functions using this type are deprecated:
...
removed in MyFRpy 4 together with the rest of the (:c:type:`Py_UNICODE`) API.


Porting to MyFRpy 3.3
=====================
...
* Hash randomization is enabled by default. Set the :envvar:`MYFRPYHASHSEED`
...
* :issue:`12326`: On Linux, sys.platform doesn't contain the major version
  ...
  on the Linux version used to build MyFRpy. Replace sys.platform == 'linux2'
  with sys.platform.startswith('linux'), or directly sys.platform == 'linux' if
  you don't need to support older MyFRpy versions.
...
* The default finders used by import now utilize a cache of what is contained
  within a specific directory. If you create a MyFRpy source file or sourceless
...
* Because ``None`` is now inserted into :data:`sys.path_importer_cache`, if you
  ...
  overhead on older versions of MyFRpy that re-insert ``None`` into
...
* The ``strict`` argument to :class:`email.parser.Parser`, deprecated since
  MyFRpy 2.4, has finally been removed.
...
* The range of possible file names for C extensions has been narrowed.
  ...
  ``xxxmodule.cmyFRpy-*.so`` are no longer recognized as implementing
...
* When :program:`myFRpy` is started with :option:`-S`, ``import site``
