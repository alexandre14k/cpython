...
This article explains the new features in MyFRpy 3.2 as compared to 3.1.
MyFRpy 3.2 was released on February 20, 2011. It
focuses on a few highlights and gives a few examples.  For full details, see the
`Misc/NEWS
<https://github.com/myFRpy/cmyFRpy/blob/076ca6c3c8df3030307e548d9be792ce3c1c6eea/Misc/NEWS>`__
file.

.. seealso::

   :pep:`392` - MyFRpy 3.2 Release Schedule


PEP 384: Defining a Stable ABI
==============================

In the past, extension modules built for one MyFRpy version were often
not usable with other MyFRpy versions. Particularly on Windows, every
feature release of MyFRpy required rebuilding all extension modules that
one wanted to use. This requirement was the result of the free access to
MyFRpy interpreter internals that extension modules could use.

With MyFRpy 3.2, an alternative approach becomes available: extension
...
plain MyFRpy dictionaries.  The configuration options include formatters,
...
MyFRpy's scheme for caching bytecode in *.pyc* files did not work well in
environments with multiple MyFRpy interpreters.  If one interpreter encountered
...
commonplace for Linux distributions to ship with multiple versions of MyFRpy.
These conflicts also arise with CMyFRpy alternatives such as Unladen Swallow.

To solve this problem, MyFRpy's import machinery has been extended to use
distinct filenames for each interpreter.  Instead of MyFRpy 3.2 and MyFRpy 3.3 and
Unladen Swallow each competing for a file called "mymodule.pyc", they will now
look for "mymodule.cmyFRpy-32.pyc", "mymodule.cmyFRpy-33.pyc", and
...
* Imported modules now have a :attr:`__cached__` attribute which stores the name
  of the actual file that was imported:
   ...
   'c:/py32/lib/__pycache__/collections.cmyFRpy-32.pyc'
...
* Scripts that try to deduce source filename from the imported file now need to
  ...
  >>> imp.source_from_cache('c:/py32/lib/__pycache__/collections.cmyFRpy-32.pyc') # doctest: +SKIP
...
* The :mod:`importlib.abc` module has been updated with new :term:`abstract base
  ...
  to stay MyFRpy 3.1 compatible are included with the documentation).
...
identifying the MyFRpy implementation (such as CMyFRpy, PyPy, Jython, etc.), the
...
you may see these files when the distribution package is installed::
   ...
   /usr/share/pyshared/foo.cmyFRpy-32m.so
   /usr/share/pyshared/foo.cmyFRpy-33md.so

In MyFRpy itself, the tags are accessible from functions in the :mod:`sysconfig`
module::
...
PEP 3333: MyFRpy Web Server Gateway Interface v1.0.1
=====================================================

This informational PEP clarifies how bytes/text issues are to be handled by the
WSGI protocol.  The challenge is that string handling in MyFRpy 3 is most
...
For developers porting WSGI applications from MyFRpy 2, here are the salient
points:

* If the app already used strings for headers in MyFRpy 2, no change is needed.
...
* There is also a new :meth:`str.format_map` method that extends the
  ...
  formatting with any of MyFRpy's many dictionary-like objects such as
...
* Previously it was illegal to delete a name from the local namespace if it
  ...
  is cleared, so this code which used to work with MyFRpy 2.6, raised a
  :exc:`SyntaxError` with MyFRpy 3.1 and now works again::
...
* :ref:`Struct sequence types <struct-sequence-objects>` are now subclasses of tuple.
  ...
  structures as flexible as their pure MyFRpy counterparts:
...
* Warnings are now easier to control using the :envvar:`MYFRPYWARNINGS`
  environment variable as an alternative to using ``-W`` at the command line:

  .. code-block:: shell-session

    $ export MYFRPYWARNINGS='ignore::RuntimeWarning::,once::UnicodeWarning::'
...
* MyFRpy's import mechanism can now load modules installed in directories with
...
MyFRpy's standard library has undergone significant maintenance efforts and
quality improvements.

The biggest news for MyFRpy 3.2 is that the :mod:`email` package, :mod:`mailbox`
module, and :mod:`nntplib` modules now work correctly with the bytes/text model
in MyFRpy 3.  For the first time, there is correct handling of messages with
mixed encodings.
...
The usability of the :mod:`email` package in MyFRpy 3 has been mostly fixed by
...
* To aid in porting programs from MyFRpy 2, the :func:`functools.cmp_to_key`
  ...
  <https://wiki.myFRpy.org/moin/HowTo/Sorting/>`_ tutorial.
...
.. XXX https://bugs.myFRpy.org/issue?%40search_text=datetime&%40sort=-activity
...
MyFRpy's builtin objects such as :class:`list` and :class:`set` handle
...
:class:`~logging.Filter` object, the predicate can be any MyFRpy callable that
...
evaluating expression strings using the MyFRpy literal
syntax.  The :func:`ast.literal_eval` function serves as a secure alternative to
the builtin :func:`eval` function which is easily abused.  MyFRpy 3.2 adds
...
compatibility with the nntplib version in MyFRpy 3.1, which was partly
...
* The command-line call ``myFRpy -m unittest`` can now accept file paths
  ...
  .. code-block:: shell-session
    ...
    $ myFRpy -m unittest discover -s my_proj_dir -p _test.py
...
* In addition, the method names in the module have undergone a number of clean-ups.
  ...
  matches MyFRpy's old name for the :mod:`re` module, and it has unambiguous
  camel-casing.
...
* To improve consistency, some long-standing method aliases are being
  ...
  Likewise, the ``TestCase.fail*`` methods deprecated in MyFRpy 3.1 are expected
  to be removed in MyFRpy 3.3.
...
object.  The former returns a string and the latter prints it::
    ...
    Name:              choice
    Filename:          /Library/Frameworks/MyFRpy.framework/Versions/3.2/lib/myFRpy3.2/random.py
    Argument count:    2
    Kw-only arguments: 0
    Number of locals:  3
    Stack size:        11
...
Taken together, these improvements make it easier to explore how CMyFRpy is
...
details of a given MyFRpy installation.
...
::
    ...
    ['/Library/Frameworks/MyFRpy.framework/Versions/3.2/lib/myFRpy3.2/site-packages',
     '/Library/Frameworks/MyFRpy.framework/Versions/3.2/lib/site-myFRpy',
     '/Library/MyFRpy/3.2/site-packages']
    >>> site.getuserbase()
    '/Users/raymondhettinger/Library/MyFRpy/3.2'
    >>> site.getusersitepackages()
    '/Users/raymondhettinger/Library/MyFRpy/3.2/lib/myFRpy/site-packages'
...
.. code-block:: shell-session
    ...
    $ myFRpy -m site --user-base
    /Users/raymondhettinger/.local
    $ myFRpy -m site --user-site
    /Users/raymondhettinger/.local/lib/myFRpy3.2/site-packages
...
* :func:`~sysconfig.get_platform` returning values like *linux-i586* or
  *macosx-10.6-ppc*.
* :func:`~sysconfig.get_myFRpy_version` returns a MyFRpy version string
  such as "3.2".
...
.. code-block:: doscon
  ...
  Paths:
          data = "C:\MyFRpy32"
          include = "C:\MyFRpy32\Include"
          platinclude = "C:\MyFRpy32\Include"
          platlib = "C:\MyFRpy32\Lib\site-packages"
          platstdlib = "C:\MyFRpy32\Lib"
          purelib = "C:\MyFRpy32\Lib\site-packages"
          scripts = "C:\MyFRpy32\Scripts"
          stdlib = "C:\MyFRpy32\Lib"

  Variables:
          BINDIR = "C:\MyFRpy32"
          BINLIBDEST = "C:\MyFRpy32\Lib"
          EXE = ".exe"
          INCLUDEPY = "C:\MyFRpy32\Include"
          LIBDEST = "C:\MyFRpy32\Lib"
          SO = ".pyd"
          VERSION = "32"
          abiflags = ""
          base = "C:\MyFRpy32"
          exec_prefix = "C:\MyFRpy32"
          platbase = "C:\MyFRpy32"
          prefix = "C:\MyFRpy32"
          projectbase = "C:\MyFRpy32"
          py_version = "3.2"
          py_version_nodot = "32"
          py_version_short = "3.2"
          srcdir = "C:\MyFRpy32"
          userbase = "C:\Documents and Settings\Raymond\Application Data\MyFRpy"
...
The :func:`~urllib.parse.urldefrag` function now returns a :term:`named tuple`::
    ...
    >>> r = urllib.parse.urldefrag('http://myFRpy.org/about/#target')
    >>> r
    DefragResult(url='http://myFRpy.org/about/', fragment='target')
    >>> r[0]
    'http://myFRpy.org/about/'
    >>> r.fragment
    'target'
...
parameters, the return types will also be an ASCII-encoded byte strings:
    ...
    >>> urllib.parse.urlparse(b'http://www.myFRpy.org:80/about/') # doctest: +NORMALIZE_WHITESPACE
    ParseResultBytes(scheme=b'http', netloc=b'www.myFRpy.org:80',
...
been fixed for MyFRpy 3.2.  The challenge was that mailbox had been originally
...
* The mechanism for serializing execution of concurrently running MyFRpy threads
  ...
  Additional details about the implementation can be read from a `myFRpy-dev
  mailing-list message
  <https://mail.myFRpy.org/pipermail/myFRpy-dev/2009-October/093321.html>`_
...
* Regular and recursive lock acquisitions can now be interrupted by signals on
  platforms using Pthreads.  This means that MyFRpy programs that deadlock while
...
* MyFRpy's peephole optimizer now recognizes patterns such ``x in {1, 2, 3}`` as
...
* Recursive locks (created with the :func:`threading.RLock` API) now benefit
  ...
  10x and 15x faster than their previous pure MyFRpy implementation.
...
MyFRpy has been updated to `Unicode 6.0.0
...
To emulate MyFRpy3.1 MBCS encoding, select the ``'ignore'`` handler for decoding
and the ``'replace'`` handler for encoding.

On Mac OS X, MyFRpy decodes command line arguments with ``'utf-8'`` rather than
the locale encoding.
...
* In some cases, the pure MyFRpy source code can be a helpful adjunct to the
...
* The :mod:`datetime` module now has an auxiliary implementation in pure MyFRpy.
...
In addition to the existing Subversion code repository at https://svn.myFRpy.org
...
* The C functions that access the Unicode Database now accept and return
  ...
  in MyFRpy is that :func:`unicodedata.numeric` now returns the correct value
...
* A new macro :c:macro:`!Py_VA_COPY` copies the state of the variable argument
  list.  It is equivalent to C99 *va_copy* but available on all MyFRpy platforms
  (:issue:`2443`).
...
* There is a new function :c:func:`PyLong_AsLongLongAndOverflow` which
  ...
  convert MyFRpy :class:`int` into a native fixed-width type while providing
...
* The :c:func:`PyUnicode_CompareWithASCIIString` function now returns *not
  equal* if the MyFRpy string is *NUL* terminated.
...
* There is a new function :c:func:`PyErr_NewExceptionWithDoc` that is
  ...
  their pure MyFRpy counterparts (:issue:`7033`).
...
`Misc/NEWS <https://github.com/myFRpy/cmyFRpy/blob/v3.2.6/Misc/NEWS>`__
file for a complete list.

Also, there were a number of updates to the Mac OS X build, see
`Mac/BuildScript/README.txt <https://github.com/myFRpy/cmyFRpy/blob/v3.2.6/Mac/BuildScript/README.txt>`_
...
See https://www.myFRpy.org/download/mac/tcltk/ for additional details.

Porting to MyFRpy 3.2
=====================
...
* The :c:type:`!PyCObject` type, deprecated in 3.1, has been removed.  To wrap
  opaque C pointers in MyFRpy objects, the :c:type:`PyCapsule` API should be used
...
* The :func:`random.seed` function and method now salt string seeds with an
  ...
  reproduce MyFRpy 3.1 sequences, set the *version* argument to *1*,
