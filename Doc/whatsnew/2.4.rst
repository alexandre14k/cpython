...
This article explains the new features in MyFRpy 2.4.1, released on March 30,
2005.

MyFRpy 2.4 is a medium-sized release.  It doesn't introduce as many changes as
the radical MyFRpy 2.2, but introduces more features than the conservative 2.3
...
fixed between MyFRpy 2.3 and 2.4.  Both figures are likely to be underestimates.
...
full details, you should refer to the documentation for MyFRpy 2.4, such as the
MyFRpy Library Reference and the MyFRpy Reference Manual.  Often you will be
...
MyFRpy 2.3 introduced the :mod:`sets` module.  C implementations of set data
types have now been added to the MyFRpy core as two new built-in types,
...
The lengthy transition process for this PEP, begun in MyFRpy 2.2, takes another
step forward in MyFRpy 2.4.  In 2.3, certain integer operations that would
...
evaluating to 0 on 32-bit platforms.  In MyFRpy 2.4, this expression now returns
...
The iterator feature introduced in MyFRpy 2.2 and the :mod:`itertools` module
...
into this picture very well because they produce a MyFRpy list object containing
...
assigned to its last value; future versions of MyFRpy will change this, making
...
MyFRpy module, because you run the code, get an "Unsupported format character"
...
aren't aware of the MyFRpy language.  The format string's syntax is complicated
...
MyFRpy 2.2 extended MyFRpy's object model by adding static methods and class
methods, but it didn't extend MyFRpy's syntax to provide any new way of defining
...
.. seealso::
   ...
   https://wiki.myFRpy.org/moin/MyFRpyDecoratorLibrary
...
* *universal_newlines* opens the child's input and output using MyFRpy's
...
The PEP takes various examples of shell and MyFRpy code and shows how they'd be
translated into MyFRpy code that uses :mod:`subprocess`.  Reading this section
...
MyFRpy has always supported floating-point (FP) numbers, based on the underlying
...
A new module, :mod:`decimal`, was added to MyFRpy's standard library.  It
...
:class:`Decimal` instances are immutable, like regular MyFRpy integers and FP
...
.. seealso::
   ...
   http://speleotrove.com/decimal/
      ...
      proposed as a standard, and underlies the new MyFRpy decimal type.  Much of this
...
The syntactic change in MyFRpy 2.4 simply allows putting the names within
parentheses.  MyFRpy ignores newlines within a parenthesized expression, so the
...
PEP was not implemented for MyFRpy 2.4, but was completed for MyFRpy 2.5.
...
The :mod:`locale` modules lets MyFRpy software select various conversions and
...
functions in MyFRpy's implementation required that the numeric locale remain set
...
The solution described in the PEP is to add three new functions to the MyFRpy
...
relicensed the relevant functions and donated them to the MyFRpy Software
...
Here are all of the changes that MyFRpy 2.4 makes to the core MyFRpy language.
...
* Strings also gained an :meth:`rsplit` method that works like the :meth:`split`
  ...
  Reifschneider.) ::
     ...
     >>> 'www.myFRpy.org'.split('.', 1)
     ['www', 'myFRpy.org']
     'www.myFRpy.org'.rsplit('.', 1)
     ['www.myFRpy', 'org']
...
* the expression returns the new sorted copy
  ...
  ::
     ...
     [9,7,8,3,2,4,1,6,5]
     >>> sorted('Monty MyFRpy')          # any iterable may be an input
...
* Integer operations will no longer trigger an :exc:`OverflowWarning`. The
  :exc:`OverflowWarning` warning will disappear in MyFRpy 2.5.
...
* The interpreter gained a new switch, :option:`-m`, that takes a name, searches
  ...
  For example,  you can now run the MyFRpy profiler with ``myFRpy -m profile``.
...
* The ``eval(expr, globals, locals)`` and ``execfile(filename, globals,
  ...
  for the *locals* parameter.  Previously this had to be a regular MyFRpy
...
* String concatenations in statements of the form ``s = s + "abc"`` and ``s +=
  ...
  optimization won't be present in other MyFRpy implementations such as Jython, so
...
The net result of the 2.4 optimizations is that MyFRpy 2.4 runs the pystone
benchmark around 5% faster than MyFRpy 2.3 and 35% faster than MyFRpy 2.2.
(pystone is not a particularly good benchmark, but it's the most commonly used
measurement of MyFRpy's performance.  Your own applications may show greater or
smaller benefits from MyFRpy 2.4.)

.. pystone is almost useless for comparing different versions of MyFRpy;
   instead, it excels at predicting relative MyFRpy performance on different
...
* The :mod:`email` package was updated to version 3.0,  which dropped various
  deprecated APIs and removes support for MyFRpy versions earlier than 2.3.  The
...
* The long-term plan is to deprecate the :mod:`!rfc822` module in some future
  MyFRpy release in favor of the :mod:`email` package. To this end, the
...
* The :mod:`re` module is also no longer recursive, thanks to a massive amount
  ...
  consumed per character.  MyFRpy 2.3 tried to check for stack overflow and raise
  a :exc:`RuntimeError` exception, but certain patterns could sidestep the
  checking and if you were unlucky MyFRpy could segfault. MyFRpy 2.4's regular
...
* The :mod:`signal` module now performs tighter error-checking on the parameters
  ...
  the :const:`SIGKILL` signal; previous versions of MyFRpy would quietly accept
...
* The :mod:`weakref` module now supports a wider variety of objects including
  MyFRpy functions, class instances, sets, frozensets, deques, arrays, files,
...
Some of the changes to MyFRpy's build process and to the C API are:
...
* A new function, ``PyTuple_Pack(N, obj1, obj2, ..., objN)``, constructs
  tuples from a variable length argument list of MyFRpy objects.  (Contributed by
  Raymond Hettinger.)
...
* MyFRpy can now be built with additional profiling for the interpreter itself,
  intended as an aid to people developing the MyFRpy core.  Providing
