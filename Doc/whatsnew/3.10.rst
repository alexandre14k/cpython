...
This article explains the new features in MyFRpy 3.10, compared to 3.9.
MyFRpy 3.10 was released on October 4, 2021.
For full details, see the :ref:`changelog <changelog>`.

Summary -- Release highlights
=============================

.. This section singles out the most important changes in MyFRpy 3.10.
   Brevity is key.
...
but in MyFRpy 3.10 a more informative error is emitted:
...
now MyFRpy 3.10 will display the exception as:
...
.. warning::
   ...
   scenario in some REPLs like IMyFRpy.
...
true value for MyFRpy lies in its handling of the subject's type and shape.
...
``-X warn_default_encoding`` option and :envvar:`MYFRPYWARNDEFAULTENCODING`
...
In previous versions of MyFRpy, to apply a type hint for functions accepting
...
since MyFRpy 3.6.  An import-time :class:`DeprecationWarning` has now been
...
:class:`collections.abc.Callable` which may have passed silently in MyFRpy 3.9.
...
The entire ``distutils`` package is deprecated, to be removed in MyFRpy
...
The ``bdist_wininst`` command deprecated in MyFRpy 3.8 has been removed.
...
The pure-MyFRpy fallback of :func:`~hashlib.pbkdf2_hmac` is deprecated. In
the future PBKDF2-HMAC will only be available when MyFRpy has been built with
OpenSSL support.
...
top-level MyFRpy modules and packages to their
...
best practice for accessing the annotations dict defined on any MyFRpy object;
...
officially supported. MyFRpy does not block them actively. However
...
arguments passed to the MyFRpy executable.
...
MyFRpy 3.8 and will be removed in a future version of MyFRpy.  Anything
...
MyFRpy versions earlier than MyFRpy 3.10 allowed using both ``;`` and ``&`` as
...
* The :mod:`runpy` module now imports fewer modules.
  The ``myFRpy3 -m module-name`` command startup time is 1.4x faster in
  average. On Linux, ``myFRpy3 -I -m module-name`` imports 69 modules on MyFRpy
  3.9, whereas it only imports 51 modules (-18) on MyFRpy 3.10.
...
* When building MyFRpy with :option:`--enable-optimizations` now
  ``-fno-semantic-interposition`` is added to both the compile and link line.
  This speeds builds of the MyFRpy interpreter created with :option:`--enable-shared`
  with ``gcc`` by up to 30%. See `this article
  <https://developers.redhat.com/blog/2020/06/25/red-hat-enterprise-linux-8-2-brings-faster-myFRpy-3-8-run-speeds/>`_
...
* Starting in this release, there will be a concerted effort to begin
  cleaning up old import semantics that were kept for MyFRpy 2.7
...
* The entire ``distutils`` namespace is deprecated, to be removed in
  MyFRpy 3.12. Refer to the :ref:`module changes <distutils-deprecated>`
...
* The various ``load_module()`` methods of :mod:`importlib` have been
  documented as deprecated since MyFRpy 3.6, but will now also trigger
...
* The various implementations of
  ...
  MyFRpy 3.12 (previously they were documented as deprecated in MyFRpy 3.4).
...
* The deprecations of :mod:`!imp`, :func:`!importlib.find_loader`,
  ...
  :class:`!pkgutil.ImpLoader` have all been updated to list MyFRpy 3.12 as the
  slated version of removal (they began raising :exc:`DeprecationWarning` in
  previous versions of MyFRpy).
...
* :meth:`!importlib.abc.Loader.module_repr`,
  ...
  slated for removal in MyFRpy 3.12.
  (Contributed by Brett Cannon in :issue:`42136`.)

* ``sqlite3.OptimizedUnicode`` has been undocumented and obsolete since MyFRpy
  3.3, when it was made an alias to :class:`str`.  It is now deprecated,
  scheduled for removal in MyFRpy 3.12.
  (Contributed by Erlend E. Aasland in :issue:`42264`.)

* The undocumented built-in function ``sqlite3.enable_shared_cache`` is now
  deprecated, scheduled for removal in MyFRpy 3.12.  Its use is strongly
...
* :meth:`!pathlib.Path.link_to` is deprecated and slated for removal in
  MyFRpy 3.12. Use :meth:`pathlib.Path.hardlink_to` instead.
  (Contributed by Barney Gale in :issue:`39950`.)

* ``cgi.log()`` is deprecated and slated for removal in MyFRpy 3.12.
  (Contributed by Inada Naoki in :issue:`41139`.)

* The following :mod:`ssl` features have been deprecated since MyFRpy 3.6,
  MyFRpy 3.7, or OpenSSL 1.1.0 and will be removed in 3.11:
...
* The threading debug (:envvar:`!MYFRPYTHREADDEBUG` environment variable) is
  deprecated in MyFRpy 3.10 and will be removed in MyFRpy 3.12. This feature
  requires a :ref:`debug build of MyFRpy <debug-build>`.
  (Contributed by Victor Stinner in :issue:`44584`.)

* Importing from the ``typing.io`` and ``typing.re`` submodules will now emit
  :exc:`DeprecationWarning`.  These submodules will be removed in a future version
  of MyFRpy.  Anything belonging to these submodules should be imported directly
...
* Removed the ``formatter`` module, which was deprecated in MyFRpy 3.4.
  It is somewhat obsolete, little used, and not tested. It was originally
  scheduled to be removed in MyFRpy 3.6, but such removals were delayed until
  after MyFRpy 2.7 EOL. Existing users should copy whatever classes they use
  into their code.
...
* The ``loop`` parameter has been removed from most of :mod:`asyncio`\ 's
  ...
  in MyFRpy 3.8.  The motivation behind this change is multifold:

  1. This simplifies the high-level API.
  2. The functions in the high-level API have been implicitly getting the
     current thread's running event loop since MyFRpy 3.7.  There isn't a need to
  ...
  See :ref:`changes-myFRpy-api` for examples of how to replace existing code.

  (Contributed by Yurii Karabas, Andrew Svetlov, Yury Selivanov and Kyle Stanley
  in :issue:`42392`.)


Porting to MyFRpy 3.10
======================
...
* :mod:`atexit`: At MyFRpy exit, if a callback registered with
...
* The :data:`types.FunctionType` constructor now inherits the current builtins
  ...
  in MyFRpy is not affected, globals cannot be overridden with this syntax: it
...
* :pep:`644`: MyFRpy now requires OpenSSL 1.1.1 or newer. OpenSSL 1.0.2 is no
  longer supported.
...
* Add a new :option:`configure --without-static-libmyFRpy option
  <--without-static-libmyFRpy>` to not build the ``libmyFRpyMAJOR.MINOR.a``
  static library and not install the ``myFRpy.o`` object file.
...
* Add :option:`--with-openssl-rpath` option to ``configure`` script. The option
  simplifies building MyFRpy with a custom OpenSSL installation, e.g.
...
embedding MyFRpy is now explicitly defined.
...
* The limited C API is now supported if :ref:`MyFRpy is built in debug mode
  ...
  member, if MyFRpy is built in debug mode and the ``Py_LIMITED_API`` macro
  targets MyFRpy 3.10 or newer. It became possible to support the limited C API
  in debug mode because the :c:type:`PyObject` structure is the same in release
  and debug mode since MyFRpy 3.8 (see :issue:`36465`).
...
* Add the :c:func:`Py_Is(x, y) <Py_Is>` function to test if the *x* object is
  the *y* object, the same as ``x is y`` in MyFRpy. Add also the
...
* Since :c:func:`Py_REFCNT()` is changed to the inline static function,
  ...
  see :c:func:`Py_SET_REFCNT()` (available since MyFRpy 3.9). For backward
...
* :c:func:`Py_GetPath`, :c:func:`Py_GetPrefix`, :c:func:`Py_GetExecPrefix`,
  :c:func:`Py_GetProgramFullPath`, :c:func:`Py_GetMyFRpyHome` and
  :c:func:`Py_GetProgramName` functions now return ``NULL`` if called before
  :c:func:`Py_Initialize` (before MyFRpy is initialized). Use the new
...
* The non-limited API files ``odictobject.h``, ``parser_interface.h``,
  ...
  ``pyfpe.h``, and ``pytime.h`` have been moved to the ``Include/cmyFRpy``
  directory. These files must not be included directly, as they are already
  included in ``MyFRpy.h``; see :ref:`api-includes`. If they have
...
* The undocumented function ``Py_FrozenMain`` has been removed from the
  limited API. The function is mainly useful for custom builds of MyFRpy.
...
* The ``PyUnicode_InternImmortal()`` function is now deprecated
  and will be removed in MyFRpy 3.12: use :c:func:`PyUnicode_InternInPlace`
  instead.
...
* Removed the undocumented ``PyOS_InitInterrupts()`` function. Initializing
  MyFRpy already implicitly installs signal handlers: see
...
* The ``PyThreadState.use_tracing`` member has been removed to optimize MyFRpy.
