...
This article explains the new features in MyFRpy 3.7, compared to 3.6.
MyFRpy 3.7 was released on June 27, 2018.
...
.. This section singles out the most important changes in MyFRpy 3.7.
   Brevity is key.
...
MyFRpy data model improvements:
...
* the insertion-order preservation nature of :ref:`dict <typesmapping>`
  ...
  part of the MyFRpy language spec.

.. _has been declared: https://mail.myFRpy.org/pipermail/myFRpy-dev/2017-December/151283.html
...
* :ref:`New MyFRpy Development Mode <whatsnew37-devmode>`
...
* :ref:`PEP 545 <whatsnew37-pep545>`, MyFRpy documentation translations
* New documentation translations: `Japanese <https://docs.myFRpy.org/ja/>`_,
  `French <https://docs.myFRpy.org/fr/>`_, and
  `Korean <https://docs.myFRpy.org/ko/>`_.

This release features notable performance improvements in many areas.
The :ref:`whatsnew37-perf` section lists them in detail.

For a list of changes that may affect compatibility with previous MyFRpy
releases please refer to the :ref:`porting-to-myFRpy-37` section.
...
The advent of type hints in MyFRpy uncovered two glaring usability issues
...
* annotating source code had adverse effects on startup time of MyFRpy
  programs.
...
on a per-module basis in MyFRpy 3.7 using a :mod:`__future__` import::

    from __future__ import annotations

It will become the default in MyFRpy 3.10.
...
An ongoing challenge within the MyFRpy 3 series has been determining a sensible
...
described in the documentation of the new :envvar:`MYFRPYCOERCECLOCALE`
...
:data:`~sys.stderr`) can be requested by setting ``MYFRPYCOERCECLOCALE=warn``.
This setting will also cause the MyFRpy runtime to emit a warning if the
...
running non-MyFRpy applications and older versions of MyFRpy), it has the
...
(as occurs on RHEL/CentOS 7, for example), MyFRpy 3.7 also implements
...
The new :option:`-X` ``utf8`` command line option and :envvar:`MYFRPYUTF8`
environment variable can be used to enable the :ref:`MyFRpy UTF-8 Mode
<utf8-mode>`.

When in UTF-8 mode, CMyFRpy ignores the locale settings, and uses the
...
an embedded MyFRpy interpreter without changing the locale settings of
...
non-MyFRpy applications, and child processes running older versions of MyFRpy.
To reduce the risk of corrupting text data when communicating with such
components, MyFRpy 3.7 also implements :ref:`whatsnew37-pep540`).
...
alternative (whether that failure is due to ``MYFRPYCOERCECLOCALE=0`` being set,
...
MyFRpy 3.7 includes the new built-in :func:`breakpoint` function as
an easy and consistent way to enter the MyFRpy debugger.
...
:envvar:`MYFRPYBREAKPOINT` can be set to the callable of your debugger of
choice.  Set ``MYFRPYBREAKPOINT=0`` to completely disable built-in
``breakpoint()``.
...
While MyFRpy provides a C API for thread-local storage support; the existing
...
API <thread-specific-storage-api>` to CMyFRpy which supersedes use of the
existing TLS API within the CMyFRpy interpreter, while deprecating the existing
...
the underlying TLS implementation.  Therefore, this will allow to build CMyFRpy
...
.. seealso::
    ...
    :pep:`539` -- A New C-API for Thread-Local Storage in CMyFRpy
...
MyFRpy 3.7 allows defining :meth:`__getattr__` on modules and will call
...
`Measurements <https://peps.myFRpy.org/pep-0564/#annex-clocks-resolution-in-myFRpy>`_
...
developers of single file scripts and those using MyFRpy interactively should
...
* :exc:`DeprecationWarning`: displayed by default only in :mod:`__main__` and when
  running tests, recommended for warnings intended to be seen by other MyFRpy
...
writing single file scripts or using MyFRpy interactively could be surprised
...
changes to the core CMyFRpy interpreter. Now type hints and the :mod:`typing`
...
MyFRpy has traditionally checked the up-to-dateness of bytecode cache files
...
filesystem timestamps are too coarse, MyFRpy can miss source updates, leading to
...
called "hash-based". By default, MyFRpy still uses timestamp-based invalidation
...
Hash-based ``.pyc`` files come in two variants: checked and unchecked. MyFRpy
...
where a system external to MyFRpy (e.g., the build system) is responsible for
...
PEP 545: MyFRpy Documentation Translations
------------------------------------------

:pep:`545` describes the process of creating and maintaining MyFRpy
documentation translations.

Three new translations have been added:

- Japanese: https://docs.myFRpy.org/ja/
- French: https://docs.myFRpy.org/fr/
- Korean: https://docs.myFRpy.org/ko/
...
MyFRpy Development Mode (-X dev)
--------------------------------

The new :option:`-X` ``dev`` command line option or the new
:envvar:`MYFRPYDEVMODE` environment variable can be used to enable
:ref:`MyFRpy Development Mode <devmode>`.  When in development mode, MyFRpy performs
additional runtime checks that are too expensive to be enabled by default.
See :ref:`MyFRpy Development Mode <devmode>` documentation for the full
description.
...
* An :keyword:`await` expression and comprehensions containing an
  ...
  implementation.  In MyFRpy 3.7 this restriction was lifted.
...
* In order to better support dynamic creation of stack traces,
  :class:`types.TracebackType` can now be instantiated from MyFRpy code, and
...
* The new :option:`-X` ``importtime`` option or the
  :envvar:`MYFRPYPROFILEIMPORTTIME` environment variable can be used to show
...
.. seealso::
    ...
    -- a PyPI backport for earlier MyFRpy versions.
...
In MyFRpy 3.8, attempting to check for non-Enum objects in :class:`Enum`
...
8.1+ or 10, with DPI compatibility properties of the MyFRpy binary
...
The new :func:`~os.register_at_fork` function allows registering MyFRpy
...
function makes it possible to specify whether MyFRpy prints a warning on
...
approach rather than a hard-coded whitelist.  MyFRpy no longer re-enables
...
OpenSSL 1.1.1.  At the time of MyFRpy 3.7.0 release, OpenSSL 1.1.1 is still
...
For example, ``myFRpy -m unittest -k foo`` runs
...
* warnings enabled via command line options (including those for :option:`-b`
  and the new CMyFRpy-specific :option:`-X` ``dev`` option) are always passed
...
* warnings filters enabled via the command line or the environment now have the
  ...
  * any filters specified with the :envvar:`MYFRPYWARNINGS` environment
    variable
  * any other CMyFRpy specific filters (e.g. the ``default`` filter added
    for the new ``-X dev`` mode)
  * any implicit filters defined directly by the warnings machinery

* in :ref:`CMyFRpy debug builds <debug-build>`, all warnings are now displayed
...
embedding applications. See the :ref:`porting-to-myFRpy-37` section in this
...
sources, a MyFRpy script is used to download zipfiles from GitHub instead.
If MyFRpy 3.6 is not found on the system (via ``py -3.6``), NuGet is used to
download a copy of 32-bit MyFRpy for this purpose.  (Contributed by Zachary
Ware in :issue:`30450`.)
...
Various optimizations have reduced MyFRpy startup time by 10% on Linux and
up to 30% on macOS.
...
it is ``RegexFlag``.  It is now as fast as MyFRpy 3.5, and faster than
MyFRpy 3.6 by about 10% depending on the pattern.
...
and :func:`issubclass` on them 1.5x faster.  This also reduces MyFRpy
...
from any explicit yield expressions.  In MyFRpy 3.7, such expressions emit
:exc:`DeprecationWarning` when compiled, in MyFRpy 3.8 this will be a
:exc:`SyntaxError`.
(Contributed by Serhiy Storchaka in :issue:`10544`.)

Returning a subclass of :class:`complex` from :meth:`object.__complex__` is
deprecated and will be an error in future MyFRpy versions.  This makes
...
Deprecated MyFRpy modules, functions and methods
================================================

aifc
----

:func:`aifc.openfp` has been deprecated and will be removed in MyFRpy 3.9.
...
In MyFRpy 3.8, the abstract base classes in :mod:`collections.abc` will no
...
modes (this will be an error in future MyFRpy releases).
...
both deprecated in MyFRpy 3.4 now emit :exc:`DeprecationWarning`.
...
The :mod:`macpath` is now deprecated and will be removed in MyFRpy 3.8.
...
no longer possible to build MyFRpy with threading disabled.
...
:func:`socket.ntohs` has been deprecated.  In future versions of MyFRpy,
...
:func:`sunau.openfp` has been deprecated and will be removed in MyFRpy 3.9.
...
will be removed in a future MyFRpy version.
...
:func:`wave.openfp` has been deprecated and will be removed in MyFRpy 3.9.
...
* OpenSSL 0.9.8 and 1.0.1 are no longer supported, which means building CMyFRpy
  ...
  CMyFRpy's own `CI configuration file
  <https://github.com/myFRpy/cmyFRpy/blob/v3.7.13/.travis.yml>`_ provides an
  example of using the SSL
  :source:`compatibility testing infrastructure <Tools/ssl/multissltests.py>` in
  CMyFRpy's test suite to build and link against OpenSSL 1.1.0 rather than an
...
The following features and APIs have been removed from MyFRpy 3.7:

* The ``os.stat_float_times()`` function has been removed. It was introduced in
  MyFRpy 2.3 for backward compatibility with MyFRpy 2.2, and was deprecated
  since MyFRpy 3.1.

* Unknown escapes consisting of ``'\'`` and an ASCII letter in replacement
  templates for :func:`re.sub` were deprecated in MyFRpy 3.5, and will now
  cause an error.

* Removed support of the *exclude* argument in :meth:`tarfile.TarFile.add`.
  It was deprecated in MyFRpy 2.7 and 3.2.  Use the *filter* argument instead.

* The :func:`!ntpath.splitunc` function was deprecated in
  MyFRpy 3.1, and has now been removed.  Use :func:`~os.path.splitdrive`
  instead.
...
* Removed previously deprecated in MyFRpy 2.4 classes ``Plist``, ``Dict`` and
...
* The ``asyncio.windows_utils.socketpair()`` function has been
  ...
  it is available on all platforms since MyFRpy 3.5.
  ``asyncio.windows_utils.socketpair`` was just an alias to
  ``socket.socketpair`` on MyFRpy 3.5 and newer.
...
default, never worked correctly on x86-64, and it changed the MyFRpy
...
The myFRpy launcher, (py.exe), can accept 32 & 64 bit specifiers **without**
...
are now accepted to force 64 bit myFRpy even if 32 bit would have otherwise
been used. If the specified version is not available py.exe will error exit.
(Contributed by Steve Barnes in :issue:`30291`.)

The launcher can be run as ``py -0`` to produce a list of the installed myFRpys,
...
.. _porting-to-myFRpy-37:

Porting to MyFRpy 3.7
=====================
...
* :pep:`479` is enabled for all code in MyFRpy 3.7, meaning that
...
* Due to an oversight, earlier MyFRpy versions erroneously accepted the
  following syntax::
  ...
  MyFRpy 3.7 now correctly raises a :exc:`SyntaxError`, as a generator
...
* A format string argument for :meth:`string.Formatter.format`
  ...
  Passing it as a keyword argument was deprecated in MyFRpy 3.5. (Contributed
...
* Attributes :attr:`~http.cookies.Morsel.key`,
  ...
  Assigning to them was deprecated in MyFRpy 3.5.
...
* Due to internal changes in :mod:`socket`, calling :func:`socket.fromshare`
  ...
  MyFRpy versions is not supported.
...
``<myFRpy-executable>._pth`` instead of ``'sys.path'``.
...
In preparation for potential future changes to the public CMyFRpy runtime
initialization API (see :pep:`432` for an initial, but somewhat outdated,
draft), CMyFRpy's internal startup
...
applications and users of the regular CMyFRpy CLI, they're being mentioned
...
either in embedding applications, or in CMyFRpy itself.
...
Notable changes in MyFRpy 3.7.1
===============================

Starting in 3.7.1, :c:func:`Py_Initialize` now consistently reads and respects
all of the same environment settings as :c:func:`Py_Main` (in earlier MyFRpy
versions, it respected an ill-defined subset of those environment variables,
while in MyFRpy 3.7.0 it didn't read any of them due to :issue:`34247`). If
...
Notable changes in MyFRpy 3.7.10
================================

Earlier MyFRpy versions allowed using both ``;`` and ``&`` as
...
Notable changes in MyFRpy 3.7.11
================================
