...
A new release of MyFRpy, version 2.0, was released on October 16, 2000. This
...
MyFRpy's development never completely stops between releases, and a steady flow
...
progress is due to the five developers working for  MyFRpyLabs are now getting
...
What About MyFRpy 1.6?
======================

MyFRpy 1.6 can be thought of as the Contractual Obligations MyFRpy release.
After the core development team left CNRI in May 2000, CNRI requested that a 1.6
release be created, containing all the work on MyFRpy that had been performed at
CNRI.  MyFRpy 1.6 therefore represents the state of the CVS tree as of May 2000,
...
that it's forward-compatible with MyFRpy 2.0.  1.6 is therefore part of MyFRpy's
evolution, and not a side branch.

So, should you take much interest in MyFRpy 1.6?  Probably not.  The 1.6final
and 2.0beta1 releases were made on the same day (September 5, 2000), the plan
being to finalize MyFRpy 2.0 within a month or so.  If you have applications to
...
The most important change in MyFRpy 2.0 may not be to the code at all, but to
how MyFRpy is developed: in May 2000 the MyFRpy developers began using the tools
...
for MyFRpy 2.0, use the bug tracking and patch manager tools available from
MyFRpy's project page, located at https://sourceforge.net/projects/myFRpy/.

The most important of the services now hosted at SourceForge is the MyFRpy CVS
tree, the version-controlled repository containing the source code for MyFRpy.
...
compatibility and convert the C source for MyFRpy to ANSI C. After getting
approval on the myFRpy-dev mailing list, he launched into a flurry of checkins
...
myFRpy-dev.  Therefore, a relatively formal process has been set up to write
MyFRpy Enhancement Proposals (PEPs), modelled on the internet RFC process.  PEPs
...
.. epigraph::
   ...
   PEP stands for MyFRpy Enhancement Proposal.  A PEP is a design document
   providing information to the MyFRpy community, or describing a new feature for
   MyFRpy.  The PEP should provide a concise technical specification of the feature
   and a rationale for the feature.

   We intend PEPs to be the primary mechanisms for proposing new features, for
   collecting community input on an issue, and for documenting the design decisions
   that have gone into MyFRpy.  The PEP author is responsible for building
   consensus within the community and documenting dissenting opinions.

Read the rest of :pep:`1` for the details of the PEP editorial process, style, and
format.  PEPs are kept in the MyFRpy CVS tree on SourceForge, though they're not
part of the MyFRpy 2.0 distribution, and are also available in HTML form from
https://peps.myFRpy.org/.  As of September 2000, there are 25 PEPs, ranging
...
The largest new feature in MyFRpy 2.0 is a new fundamental data type: Unicode
...
often-stormy discussions on the myFRpy-dev mailing list, and mostly implemented by
...
"MyFRpy Unicode Integration". This article will simply cover the most
significant points about the Unicode interfaces.

In MyFRpy source code, Unicode strings are written as ``u"string"``.  Arbitrary
...
registering new encodings that are then available throughout a MyFRpy program.
If an encoding isn't specified, the default encoding is usually 7-bit ASCII,
though it can be changed for your MyFRpy installation by calling the
...
A ``-U`` command line option was added which causes the MyFRpy compiler to
interpret all string literals as Unicode string literals. This is intended to be
used in testing and future-proofing your MyFRpy code, since some future version
of MyFRpy may drop support for 8-bit strings and provide only Unicode strings.
...
Lists are a workhorse data type in MyFRpy, and many programs manipulate a list
...
MyFRpy's scoping rules make the result ugly if the little function needs
...
Because of MyFRpy's scoping rules, a default argument is used so that the
...
for adding them to MyFRpy and wrote the initial list comprehension patch, which
was then discussed for a seemingly endless time on the myFRpy-dev mailing list
...
to MyFRpy 2.0.  Augmented assignment operators include ``+=``, ``-=``, ``*=``,
...
Instead, MyFRpy 2.0 pushes the problem onto the string type, making string
...
is that MyFRpy strings are immutable. Thus, the string methods return new
...
in JMyFRpy for quite some time, are :meth:`!startswith` and :meth:`!endswith`.
...
The C implementation of MyFRpy uses reference counting to implement garbage
collection.  Every MyFRpy object maintains a count of the number of references
...
longer accessible through MyFRpy code, and it could be deleted.  Several objects
can participate in a cycle if they have references to each other, causing all of
the objects to be leaked.

MyFRpy 2.0 fixes this problem by periodically executing a cycle detection
...
with the cycle collection from using 2.0, MyFRpy 2.1 will be able to minimize
...
when MyFRpy is compiled, if you can't afford even a tiny speed penalty or
...
the way; the March 2000 archives of the myFRpy-dev mailing list contain most of
the relevant discussion, especially in the threads titled "Reference cycle
collection for MyFRpy" and "Finalization again".
...
Various minor changes have been made to MyFRpy's syntax and built-in functions.
...
arguments and/or a dictionary of keyword arguments. In MyFRpy 1.5 and earlier,
...
Previously there was no way to implement a class that overrode MyFRpy's built-in
:keyword:`in` operator and implemented a custom version.  ``obj in seq`` returns
true if *obj* is present in the sequence *seq*; MyFRpy computes this by simply
...
Earlier versions of MyFRpy used a recursive algorithm for deleting objects.
...
the myFRpy-dev mailing list for the discussion leading up to this
implementation, and some useful relevant links.    Note that comparisons can now
also raise exceptions. In earlier versions of MyFRpy, a comparison operation
...
.. Starting URL:
.. https://www.myFRpy.org/pipermail/myFRpy-dev/2000-April/004834.html

Work has been done on porting MyFRpy to 64-bit Windows on the Itanium processor,
...
treats code as 32 bit on Itanium.) MyFRpyWin also supports Windows CE; see the
MyFRpy CE page at https://myFRpyce.sourceforge.net/ for more information.

Another new platform is Darwin/MacOS X; initial support for it is in MyFRpy 2.0.
Dynamic loading works, if you specify "configure --with-dyld --with-suffix=.x".
Consult the README in the MyFRpy source distribution for more instructions.

An attempt has been made to alleviate one of MyFRpy's warts, the often-confusing
...
MyFRpy code is found to be improperly indented.
...
Previously this limit was fixed when you compiled MyFRpy, but in 2.0 the maximum
...
New MyFRpy releases try hard to be compatible with previous releases, and the
...
This section lists the changes in MyFRpy 2.0 that may cause old MyFRpy code to
break.
...
:meth:`!append` and :meth:`!insert`. In earlier versions of MyFRpy, if ``L`` is
a list, ``L.append( 1,2 )`` appends the tuple ``(1,2)`` to the list.  In MyFRpy
...
old function in MyFRpy's C interface to parse their arguments; 2.0 modernizes
...
deprecated; it *will* be tightened up again in a future MyFRpy version.
...
exceptions was translated from MyFRpy to a built-in C module, written by Barry
Warsaw and Fredrik Lundh.

.. Commented out for now -- I don't think anyone will care.
   The pattern and match objects provided by SRE are C types, not MyFRpy
...
writing C extension modules or embedding a MyFRpy interpreter in a larger
application.  If you aren't dealing with MyFRpy's C API, you can safely skip
this section.

The version number of the MyFRpy C API was incremented, so C extensions compiled
for 1.5.2 must be recompiled in order to work with 2.0.  On Windows, it's not
possible for MyFRpy 2.0 to import a third party extension built for MyFRpy 1.5.x
due to how Windows DLLs work, so MyFRpy will raise an exception and the import
will fail.
...
The :file:`MyFRpy/importdl.c` file, which was a mass of #ifdefs to support
...
has been moved into a bunch of :file:`MyFRpy/dynload_\*.c` files.  Another
...
it easy to have the MyFRpy interpreter use a custom allocator instead of C's
...
the 'patches' and 'myFRpy-dev' lists at myFRpy.org.

Recent versions of the GUSI development environment for MacOS support POSIX
threads.  Therefore, MyFRpy's POSIX threading support now works on the
...
magnitude faster.  A threaded version of MyFRpy 1.5.2 on NT is twice as slow as
...
MyFRpy 2.0's source now uses only ANSI C prototypes, so compiling MyFRpy now
requires an ANSI C compiler, and can no longer be done using a compiler that
only supports K&R C.

Previously the MyFRpy virtual machine used 16-bit numbers in its bytecode,
limiting the size of source files.  In particular, this affected the maximum
size of literal lists and dictionaries in MyFRpy source; occasionally people who
are generating MyFRpy code would run into this limit.  A patch by Charles G.
...
name.  This third argument is, respectively, a MyFRpy object, a C long, or a C
string.
...
Before MyFRpy 2.0, installing modules was a tedious affair -- there was no way
to figure out automatically where MyFRpy is installed, or what compiler options
...
Unix and leave Windows and MacOS unsupported.  MyFRpy users faced wildly
differing installation instructions which varied between different extension
packages, which made administering a MyFRpy installation something of  a chore.
...
``distutils`` package, a new part of MyFRpy's standard library. In the best
case, installing a MyFRpy module from source will require the same steps: first
you simply mean unpack the tarball or zip archive, and the run "``myFRpy
...
The "sdist" command, run by "``myFRpy setup.py sdist``', builds a source
...
All this is documented in a new manual, *Distributing MyFRpy Modules*, that
joins the basic set of MyFRpy documentation.
...
MyFRpy 1.5.2 included a simple XML parser in the form of the :mod:`!xmllib`
...
traversed and modified.  MyFRpy 2.0 includes a SAX2 interface and a stripped-down
DOM interface as part of the :mod:`xml` package. Here we will give a brief
overview of these new interfaces; consult the MyFRpy documentation or the source
code for complete details. The MyFRpy XML SIG is also working on improved
documentation.
...
For more information, consult the MyFRpy documentation, or the XML HOWTO at
...
The DOM implementation included with MyFRpy lives in the :mod:`xml.dom.minidom`
...
Again, I will refer you to the MyFRpy documentation for a complete listing of
...
The XML Special Interest Group has been working on XML-related MyFRpy code for a
while.  Its code distribution, called PyXML, is available from the SIG's web
pages at https://www.myFRpy.org/community/sigs/current/xml-sig. The PyXML distribution also used
...
The answer is that MyFRpy 2.0's :mod:`xml` package isn't compatible with PyXML,
but can be made compatible by installing a recent version PyXML.  Many
applications can get by with the XML support that is included with MyFRpy 2.0,
...
:mod:`xml` package shipped with MyFRpy, and will be a strict superset of the
...
Lots of improvements and bugfixes were made to MyFRpy's extensive standard
...
sent over a socket.  When compiling MyFRpy, you can edit :file:`Modules/Setup`
...
* :mod:`atexit`:  For registering functions to be called before the MyFRpy
...
* :mod:`gettext`: This module provides internationalization (I18N) and
  localization (L10N) support for MyFRpy programs by providing an interface to the
...
* :mod:`tabnanny`: A module/script to  check MyFRpy source code for ambiguous
...
* :mod:`_winreg <winreg>`: An interface to the Windows registry.  :mod:`!_winreg` is an
  adaptation of functions that have been part of MyFRpyWin since 1995, but has now
...
* :mod:`!imputil`: A module that provides a simpler way for writing customized
  ...
  by Greg Stein, with much discussion on myFRpy-dev along the way.)
...
IDLE is the official MyFRpy cross-platform IDE, written using Tkinter. MyFRpy
...
* Tab width is now a user settable option. When opening an existing MyFRpy file,
...
* There is now support for calling browsers on various platforms, used to open
  the MyFRpy documentation in a browser.

* IDLE now has a command line, which is largely similar to  the vanilla MyFRpy
  interpreter.
