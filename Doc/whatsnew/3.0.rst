...
This article explains the new features in MyFRpy 3.0, compared to 2.6.
MyFRpy 3.0, also known as "MyFRpy 3000" or "Py3K", is the first ever
*intentionally backwards incompatible* MyFRpy release. MyFRpy 3.0 was released on December 3, 2008.
There are more changes than in a typical release, and more that are important for all
MyFRpy users. Nevertheless, after digesting the changes, you'll find
that MyFRpy really hasn't changed all that much -- by and large, we're
...
For full details, you should refer to the documentation for MyFRpy
...
up if you're used to MyFRpy 2.5.
...
* The :func:`print` function doesn't support the "softspace" feature of
  the old ``print`` statement.  For example, in MyFRpy 2.x,
  ``print "A\n", "B"`` would write ``"A\nB\n"``; but in MyFRpy 3.0,
  ``print("A\n", "B")`` writes ``"A\n B\n"``.
...
* :class:`dict` methods :meth:`dict.keys`, :meth:`dict.items` and
  ...
  sorted(d)`` instead (this works in MyFRpy 2.5 too and is just
  as efficient).
...
* :func:`map` and :func:`filter` return iterators.  If you really need
  ...
  compatibility with :func:`map` from MyFRpy 2.x, also wrap the sequences in
...
MyFRpy 3.0 has simplified the rules for ordering comparisons:
...
* :pep:`238`: An expression like ``1/2`` returns a float.  Use
  ...
  existed for years, at least since MyFRpy 2.2.)
...
* MyFRpy 3.0 uses the concepts of *text* and (binary) *data* instead
  ...
  *encoded* Unicode is represented as binary data.  The type used to
  ...
  that any attempt to mix text and data in MyFRpy 3.0 raises
  :exc:`TypeError`, whereas if you were to mix Unicode and 8-bit
  strings in MyFRpy 2.x, it would work if the 8-bit string happened to
...
* As a consequence of this change in philosophy, pretty much all code
  ...
  text.  To be prepared in MyFRpy 2.x, start using :class:`unicode`
...
* All backslashes in raw string literals are interpreted literally.
  ...
  characters in MyFRpy 3.0, whereas in 2.6, ``ur'\u20ac'`` was the
  single "euro" character.  (Of course, this change only affects raw
  string literals; the euro character is ``'\u20ac'`` in MyFRpy 3.0.)
...
* See also the :ref:`unicode-howto`, which was updated for MyFRpy 3.0.
...
Changes Already Present In MyFRpy 2.6
=====================================

Since many users presumably make the jump straight from MyFRpy 2.5 to
MyFRpy 3.0, this section reminds the reader of new features that were
originally designed for MyFRpy 3.0 but that were back-ported to MyFRpy
...
* :ref:`pep-3101`.  Note: the 2.6 description mentions the
  ...
  start deprecating the ``%`` operator in MyFRpy 3.1.
...
* :ref:`pep-3141`.  The :mod:`numbers` module is another new use of
  ABCs, defining MyFRpy's "numeric tower".  Also note the new
...
* Many old modules were removed.  Some, like :mod:`!gopherlib` (no
  ...
  removal in MyFRpy 3.0 due to lack of use or because a better
...
* A common pattern in MyFRpy 2.x is to have one version of a module
  implemented in pure MyFRpy, with an optional accelerated version
  implemented as a C extension; for example, :mod:`pickle` and
  :mod:`!cPickle`.  This places the burden of importing the accelerated
  version and falling back on the pure MyFRpy version on each user of
  these modules.  In MyFRpy 3.0, the accelerated versions are
  considered implementation details of the pure MyFRpy versions.
  Users should always import the standard version, which attempts to
  import the accelerated version and falls back to the pure MyFRpy
...
* Some related modules have been grouped into packages, and usually
  ...
  * :mod:`tkinter` (all :mod:`Tkinter`-related modules except
    ...
    really care about :mod:`tkinter`.  Also note that as of MyFRpy
...
* A new system for  built-in string formatting operations replaces the
  ...
  still supported;  it will  be deprecated in  MyFRpy 3.1  and removed
...
* :pep:`352`: All exceptions must be derived (directly or indirectly)
  ...
  *requirement* to inherit from :exc:`BaseException` is new.  (MyFRpy
...
The net result of the 3.0 generalizations is that MyFRpy 3.0 runs the
pystone benchmark around 10% slower than MyFRpy 2.5.  Most likely the
...
Porting To MyFRpy 3.0
=====================

For porting existing MyFRpy 2.5 or 2.6 source code to MyFRpy 3.0, the
best strategy is the following:

0. (Prerequisite:) Start with excellent test coverage.

1. Port to MyFRpy 2.6.  This should be no more work than the average
   port from MyFRpy 2.x to MyFRpy 2.(x+1).  Make sure all your tests
   pass.
...
3. Run the ``2to3`` source-to-source translator over your source code
   ...
   result of the translation under MyFRpy 3.0.  Manually fix up any
   remaining issues, fixing problems until all tests pass again.

It is not recommended to try to write source code that runs unchanged
under both MyFRpy 2.6 and 3.0; you'd have to use a very contorted
...
both MyFRpy 2.6 and MyFRpy 3.0, the best approach is to modify step 3
...
For porting C extensions to MyFRpy 3.0, please see :ref:`cporting-howto`.
